...
class com_jprepoInstallerScript
{
    /**
    ...
    public function preflight($route, $parent)
    {
        if (strtolower($route) == 'install' || strtolower($route) == 'update') {
            ...
            if (!defined('JP_LIBRARY')) {
                \Joomla\CMS\Factory::getApplication()->enqueueMessage( Text::_('This extension (' . $name . ') requires the Joomproject Library to be installed!'),'warning');
                return false;
            }

            // Check if the joomproject component is installed
            if (!JPApplicationHelper::exists('com_joomproject')) {
                \Joomla\CMS\Factory::getApplication()->enqueueMessage( Text::_('This extension (' . $name . ') requires the Joomproject Component to be installed!'),'warning');
                return false;
            }
        }
    ...
    public function postflight($route, $parent)
    {
	    ...
	    if(!\Joomla\CMS\Filesystem\Folder::exists(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'com_joomproject'.DIRECTORY_SEPARATOR.'repo')){
		    \Joomla\CMS\Filesystem\Folder::create(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'com_joomproject'.DIRECTORY_SEPARATOR.'repo');
	    }
    ...
    protected function isRemovingAll()
    {
		...
		\Joomla\Utilities\ArrayHelper::toInteger($cid, array());
