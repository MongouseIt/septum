...
/**
 * Repository file revisions view class.
 *
 */
class JPrepoViewFileRevisions extends HtmlView
{
    ...
    protected function prepareDocument()
    {
        ...
        if ($menu) {
            ...
            $t = Text::_('COM_JOOMPROJECT_PAGE_VIEW_FILE_REVISIONS') . ': ' . $this->escape($this->item->title);
    ...
    {
        $access    = JPrepoHelper::getActions('file', $this->item->id);
        $link      = JPrepoHelperRoute::getRepositoryRoute($this->item->project_id, $this->item->dir_id, $this->item->path);
        $back_opts = array('access' => true, 'href' => $link, 'icon' => 'fas fa-share', 'class' => 'btn-danger');
        $edit_opts = array('access' => $access->get('core.edit'),'icon' => 'fas fa-edit', 'class' => 'btn-warning text-white');

        JPToolbar::button('COM_JOOMPROJECT_ACTION_BACK', '', false, $back_opts);
        JPToolbar::button('COM_JOOMPROJECT_ACTION_EDIT', 'fileform.edit', false, $edit_opts);
