...
foreach ($this->items['files'] as $i => $item) :
    $dispatcher->triggerEvent('onContentPrepare', array('com_jprepo.file', &$item, &$this->params, 0));
    $link   = JPrepoHelperRoute::getFileRoute($item->slug, $item->project_slug, $item->dir_slug, $item->path);
    $access = JPrepoHelper::getActions('file', $item->id);
    $icon   = $this->escape(strtolower($item->file_extension));


	$icon_class = JoomprojectHelperFrontend::extToIcon($icon);
    ...
    <tr class="row<?php echo $i % 2; ?>">
        <td><?php
	        ...
	        if ($item->revision_count) {
		        $link_revs = JPrepoHelperRoute::getFileRevisionsRoute($item->slug, $item->project_slug, $item->dir_slug, $item->path);
        ...
        <td class="text-center d-none d-sm-table-cell">
            <label for="cb<?php echo $x; ?>" class="checkbox p-0">
                <?php echo HTMLHelper::_('jp.html.id', $x, $item->id, false, 'fid',"mt-2 me-0"); ?>
            </label>
        </td>
        <?php endif; ?>
        <td>
        	<span class="item-title">
	            ...
	            <a  href="<?php echo Route::_($link);?>" class="" data-bs-toggle="popover"
                   title="<?php echo Text::_($this->escape($item->title)); ?>"
  ...
  data-content='<?php echo LayoutHelper::render(
                   ...
                   data-placement="right">
	            	<span aria-hidden="true" class="text-muted me-2 far fa-<?php echo $icon_class;?>"></span>
        ...
        <td class="d-none d-sm-table-cell">
        	<?php echo Text::_('JGRID_HEADING_FILE'); ?> <span class="text-muted small"><?php echo $icon; ?></span>
        </td>
        <td class="d-none d-sm-table-cell">
