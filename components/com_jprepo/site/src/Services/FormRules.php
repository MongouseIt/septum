...
namespace Joomla\Component\JPrepo\Site\Service;
...
class FormRules implements RulesInterface
{
    ...
    public function build(&$query, &$segments)
    {
        ...
        $segments = self::JPrepoBuildRoute($query);
    ...
    {
        $vars = self::JPrepoParseRoute($segments);
    ...
    /**
     * Build the route for the com_jprepo component
     *
     * @param     array    $query    An array of URL arguments
     *
     * @return    array              The URL arguments to use to assemble the subsequent URL.
     */
    function JPrepoBuildRoute(&$query)
    {
        ...
        if(in_array($view, $views)) {
            ...
            if (isset($query['filter_project'])) {
                if (strpos($query['filter_project'], ':') === false) {
                    $query['filter_project'] = self::JPrepoMakeSlug($query['filter_project'], '#__jp_projects');
                }
            }
            else {
                $query['filter_project'] = self::JPrepoMakeSlug('0', '#__jp_projects');
            }
            ...
            if (isset($query['filter_parent_id'])) {
                if (strpos($query['filter_parent_id'], ':') === false) {
                    $query['filter_parent_id'] = self::JPrepoMakeSlug($query['filter_parent_id'], '#__jp_repo_dirs');
                }
            }
            else {
                $query['filter_parent_id'] = self::JPrepoMakeSlug('0', '#__jp_repo_dirs');
            }
            ...
            if (isset($query['id']) && ($view == 'file' || $view == 'filerevisions')) {
                if (strpos($query['id'], ':') === false) {
                    $query['id'] = self::JPrepoMakeSlug($query['id'], '#__jp_repo_files');
                }
            ...
            if (isset($query['id']) && ($view == 'note' || $view == 'noterevisions')) {
                if (strpos($query['id'], ':') === false) {
                    $query['id'] = self::JPrepoMakeSlug($query['id'], '#__jp_repo_notes');
                }
     ...
     */
    function JPrepoParseRoute($segments)
    {
        // Setup vars
        $vars  = array();
        ...
        if ($vars['view'] == 'repository') {
            ...
            if ($count2 >= 1) {
                $vars['filter_project'] = self::JPrepoParseSlug($segments[0]);
            }
            if ($count2 >= 2) {
                $i    = 1;
                $path = array();
                ...
                {
                    if ($i == ($count2 - 1)) {
                        $vars['filter_parent_id'] = self::JPrepoParseSlug($segments[$i]);
                    }
                    else {
                        $path[] = self::JPrepoParseSlug($segments[$i]);
                    }
                    $i++;
                }
        ...
        if ($vars['view'] == 'file' || $vars['view'] == 'filerevisions') {
            $vars['id'] = self::JPrepoParseSlug($segments[$count - 1]);
     ...
     */
    function JPrepoParseSlug($segment)
    {
     ...
     */
   public static function JPrepoMakeSlug($id, $table, $alt = 'all', $field = 'alias')
    {
        if ($id == '' || $id == '0') {
