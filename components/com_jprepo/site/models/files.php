 ...
 *
 */
class JPrepoModelFiles extends ListModel
{
    ...
    public function getListQuery()
    {
        ...
        $filter_dir     = (int) $this->getState('filter.dir_id');
        ...
        // filter by milestone
        $filter_milestone = Factory::getApplication()->input->get('filter_milestone_id',0,'int');
        if($filter_milestone){
            $query->join('LEFT', '#__jp_ref_attachments AS at ON (at.project_id = a.project_id AND FIND_IN_SET(a.id,REPLACE(at.attachment,".",",")))');
            $query->where('at.item_type = "com_jpmilestones.milestone" AND at.item_id = '.$filter_milestone);
        }
        ...
        $query->select('COUNT(DISTINCT lbl.id) AS label_count')
              ->join('LEFT', '#__jp_ref_labels AS lbl ON (lbl.item_id = a.id AND '
                           . 'lbl.item_type = ' . $this->_db->quote('com_jprepo.file') . ')');
    ...
    protected function populateState($ordering = 'a.title', $direction = 'ASC')
    {
        ...
        // Filter - Directory
        $dir_id = \Joomla\CMS\Factory::getApplication()->input->getCmd('filter_parent_id', \Joomla\CMS\Factory::getApplication()->input->getCmd('filter_dir_id', ''));
        $this->setState('filter.dir_id', $dir_id);

        // Filter - Labels
        $labels = (array) \Joomla\CMS\Factory::getApplication()->input->post->get('filter_label',[],'array');
