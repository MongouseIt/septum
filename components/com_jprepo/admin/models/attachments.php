 ...
 *
 */
class JPrepoModelAttachments extends ListModel
{
    /**
     * Constructor
     *
    ...
    public function getItems($item_type = NULL, $item_id = 0)
    {
        ...
        if (empty($item_type) || empty($item_id)) {
            $this->setError(Text::_('COM_JOOMPROJECT_WARNING_ATTACHMENTS_NO_ITEM_REFERENCE'));
            return array();
        }
        ...
        // Load just the ID's
        $query->select('a.id')
              ->from('#__jp_ref_attachments AS a')
        ...
        $items      = array();
        $attachment = $this->getInstance('Attachment', 'JPrepoModel', array('ignore_request' => true));
