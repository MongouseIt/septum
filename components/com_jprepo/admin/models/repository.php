...
class JPrepoModelRepository extends ListModel
{
    ...
    public function getItems()
    {
        ...
        $dir    = $this->getInstance('Directory', 'JPrepoModel', $config = array('ignore_request' => true));
        ...
        if ($parent == 1) {
            ...
            try {
                ...
                if ($count) {
                    $pks      = \Joomla\Utilities\ArrayHelper::getColumn($items['directories'], 'id');
    ...
    protected function populateState($ordering = 'a.title', $direction = 'asc')
    {
        ...
        // Filter - Directory
        $parent_id = \Joomla\CMS\Factory::getApplication()->input->get('filter_parent_id');
        ...
        if (\Joomla\CMS\Factory::getApplication()->input->get('filter_project', null, 'post') === '0') {
            $parent_id = 1;
            $project   = 0;
        }
        ...
        \Joomla\CMS\Factory::getApplication()->input->set('filter_parent_id', $parent_id);
        \Joomla\CMS\Factory::getApplication()->input->set('filter_project',   $project);

        // Handle list limit
        if ($project) {
            \Joomla\CMS\Factory::getApplication()->input->set('limit', 0);
        }
        else {
            if (\Joomla\CMS\Factory::getApplication()->input->get('limit') === null) {
                \Joomla\CMS\Factory::getApplication()->input->set('limit', $app->getCfg('list_limit'));
            }
        }
