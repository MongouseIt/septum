...
JLoader::register('JPrepoHelperRoute', JPATH_SITE . '/components/com_jprepo/helpers/route.php');
 ...
 *
 */
abstract class JPrepoNotificationsHelper
{
    /**
     ...
     * @var    array
     */
    protected static $contexts = array('com_jprepo.attachment',
                                       'com_jprepo.file', 'com_jprepo.fileform',
                                       'com_jprepo.note', 'com_jprepo.noteform'
                                       );
     ...
     * @var    string
     */
    protected static $prefix   = 'COM_JOOMPROJECT_ATTACHMENT_EMAIL';
    ...
    public static function getItemName($context)
    {
        switch ($context)
        {
            case 'com_jprepo.file':
            case 'com_jprepo.fileform':
                self::$prefix = 'COM_JPREPO_FILE_EMAIL';
                return 'file';
                break;

            case 'com_jprepo.note':
            case 'com_jprepo.noteform':
                self::$prefix = 'COM_JPREPO_NOTE_EMAIL';
                return 'note';
                break;

            default:
                self::$prefix = 'COM_JOOMPROJECT_ATTACHMENT_EMAIL';
                return 'attachment';
                break;
        }
    }
    ...
    public static function getObservers($context, $table, $is_new = false)
    {
        ...
        if (in_array($context, array('com_jprepo.file', 'com_jprepo.fileform', 'com_jprepo.note', 'com_jprepo.noteform'))) {
            ...
            $query->where(
                '('
                . 'a.item_type = ' . $db->quote('com_jprepo.directory')
        ...
        if ($opt_out) {
            ...
            if (isset($table->access)) {
                $repo_groups = JPAccessHelper::getGroupsByAccessLevel($table->access);
            }
    ...
    public static function getFileMessage($lang, $receiver, $user, $after, $before, $is_new)
    {
        ...
        $link    = Route::_(Uri::root() . JPrepoHelperRoute::getFileRoute($after->id, $after->project_id, $after->dir_id));
    ...
    public static function getNoteMessage($lang, $receiver, $user, $after, $before, $is_new)
    {
        ...
        $changes = JPnotificationsHelper::formatChanges($lang, $changes);
        $footer  = sprintf($lang->_('COM_JOOMPROJECT_EMAIL_FOOTER'), Uri::root());
        $link    = Route::_(Uri::root() . JPrepoHelperRoute::getNoteRoute($after->id, $after->project_id, $after->dir_id));
        $txt     = sprintf($format, $receiver->name, $user->name, $changes, $link);
        $txt     = str_replace('\n', "\n", $txt . "\n\n" . $footer);
