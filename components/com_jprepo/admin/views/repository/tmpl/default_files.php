...
$txt_icon    = Text::_('COM_JOOMPROJECT_FIELD_FILE_LABEL');
$date_format = Text::_('DATE_FORMAT_LC4');
$txt_dl      = Text::_('COM_JOOMPROJECT_DOWNLOAD');
...
foreach ($this->items['files'] as $i => $item) :
    ...
    $cm_dl = 'index.php?option=com_jprepo&task=file.download'
           . '&filter_project=' . $item->project_id . 'filter_parent_id=' . $item->dir_id . '&id=' . $item->id;
    ?>
    <tr class="row<?php echo $i % 2; ?>">
        <td class="text-center d-none d-sm-table-cell">
            <?php echo HTMLHelper::_('grid.id', $i, $item->id, false, 'fid'); ?>
        </td>
        <td class="has-context">
            <div class="float-start fltlft">
                ...
                <i class="fas fa-flag" data-bs-toggle="tooltip" title="<?php echo $txt_icon;?>"></i>
            ...
            <div class="float-start">
                ...
                HTMLHelper::_('dropdown.edit', $item->id, 'file.');
                HTMLHelper::_('dropdown.addCustomItem', $txt_dl, Route::_($cm_dl));

                if ($item->revision_count) {
                    $cm_revs = 'index.php?option=com_jprepo&view=filerevisions'
                        . '&filter_project=' . $item->project_id . 'filter_parent_id=' . $item->dir_id . '&id=' . $item->id;
