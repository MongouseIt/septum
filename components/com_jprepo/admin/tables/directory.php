...
JLoader::register('JPtableDirectoryCompat', dirname(__FILE__) . '/directory_compat.php');
 ...
 *
 */
class JPTableDirectory extends JPtableDirectoryCompat
{
    /**
     * Constructor
     *
    ...
    {
        parent::__construct('#__jp_repo_dirs', 'id', $db);
    }
    ...
    public function check()
    {
        ...
        if (trim(str_replace('-','', $this->alias)) == '') $this->alias = Joomla\CMS\Application\ApplicationHelper::stringURLSafe(Factory::getDate()->format('Y-m-d-H-i-s'));
    ...
    {
        $k = $this->_tbl_key;
        return 'com_jprepo.directory.' . (int) $this->$k;
    }
    ...
    protected function _getAssetParentIdCompat($table = null, $id = null)
    {
        ...
        if ($this->parent_id > 1) {
            ...
            $query->select($this->_db->quoteName('asset_id'))
                  ->from($this->_db->quoteName('#__jp_repo_dirs'))
        ...
        elseif ($this->parent_id == 1) {
            // This is a project root directory
            $query->select($this->_db->quoteName('id'))
                  ->from($this->_db->quoteName('#__assets'))
                  ->where($this->_db->quoteName('name') . ' = ' . $this->_db->quote('com_jprepo.project.' . $this->project_id));
