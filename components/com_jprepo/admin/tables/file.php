...
require_once JPATH_ADMINISTRATOR . '/components/com_jprepo/helpers/jprepo.php';

// Register compat class
JLoader::register('JPtableFileCompat', dirname(__FILE__) . '/file_compat.php');
 ...
 *
 */
class JPtableFile extends JPtableFileCompat
{
    /**
     * Constructor
     *
    ...
    {
        parent::__construct('#__jp_repo_files', 'id', $db);
    }
    ...
    {
        $k = $this->_tbl_key;
        return 'com_jprepo.file.' . (int) $this->$k;
    }
    ...
    public function deleteReferences($pk = null)
    {
        ...
        $path = JPrepoHelper::getFilePath($this->file_name, $this->dir_id);

        if (!empty($path)) {
            \Joomla\CMS\Filesystem\File::delete($path . '/' . $this->file_name);
        }

        // Delete the revisions folder
        $path = JPrepoHelper::getBasePath($this->project_id) . '/_revs/file_' . (int) $pk;
