...
JLoader::register('JPtableRevisionCompat', dirname(__FILE__) . '/revision_compat.php');
 ...
 *
 */
class JPtableRevision extends JPtableRevisionCompat
{
    /**
     * Constructor
     *
    ...
    {
        parent::__construct('#__jp_design_revisions', 'id', $db);
    ...
    {
        $k = $this->_tbl_key;
        return 'com_jpdesigns.revision.' . (int) $this->$k;
    }
    ...
    public function check()
    {
        ...
        if ((int) $this->parent_id <= 0) {
            $this->setError(Text::_('COM_JOOMPROJECT_WARNING_SELECT_DESIGN'));
            return false;
        }
    ...
    public function store($updateNulls = false)
    {
        ...
        $table = Table::getInstance('Revision', 'JPtable');
        $data  = array('project_id' => $this->project_id, 'parent_id' => $this->parent_id, 'alias' => $this->alias);

        if ($table->load($data) && ($table->id != $this->id || $this->id == 0)) {
            $this->setError(Text::_('COM_JOOMPROJECT_ERROR_DESIGN_REVISION_UNIQUE_ALIAS'));
            return false;
        }
    ...
    public function deleteReferences($pk = null)
    {
        ...
        $query->delete('#__jp_designs_approved')
              ->where('revision_id = ' . $this->_db->quote((int) $pk));
        ...
        if ($media) {
            $model = BaseDatabaseModel::getInstance('Revision', 'JPdesignsModel', array('ignore_request' => true));
