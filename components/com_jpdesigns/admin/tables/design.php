...
JLoader::register('JPdesignsModelDesign', JPATH_ADMINISTRATOR . '/components/com_jpdesigns/models/design.php');

// Register compat class
JLoader::register('JPtableDesignCompat', dirname(__FILE__) . '/design_compat.php');
 ...
 *
 */
class JPtableDesign extends JPtableDesignCompat
{
    /**
     * Constructor
     *
    ...
    {
        parent::__construct('#__jp_designs', 'id', $db);
    ...
    {
        $k = $this->_tbl_key;
        return 'com_jpdesigns.design.' . (int) $this->$k;
    }
    ...
    protected function _getAssetParentIdCompat($table = null, $id = null)
    {
        ...
        if ($this->album_id) {
            // This is a design in an album
            $query->select('asset_id')
                  ->from('#__jp_design_albums')
                  ->where('id = ' . (int) $this->album_id);
        ...
        if(!$asset_id && version_compare(JPVERSION, '4.2', 'ge')) {
            // Fall back to component project asset
            $query->clear();
            $query->select($this->_db->quoteName('id'))
                  ->from($this->_db->quoteName('#__assets'))
                  ->where($this->_db->quoteName('name') . ' = ' . $this->_db->quote("com_jpdesigns.project." . (int) $this->project_id));
        ...
        if(!$asset_id) {
            ...
            $query->select($this->_db->quoteName('id'))
                  ->from($this->_db->quoteName('#__assets'))
                  ->where($this->_db->quoteName('name') . ' = ' . $this->_db->quote("com_jpdesigns"));
    ...
    protected function _getParentAccess()
    {
        ...
        $album   = (int) $this->album_id;
        $project = (int) $this->project_id;

        if ($album > 0) {
            $query->select('access')
                  ->from('#__jp_design_albums')
                  ->where('id = ' . $db->quote($album));
        }
        elseif ($project > 0) {
            $query->select('access')
                  ->from('#__jp_projects')
    ...
    public function store($updateNulls = false)
    {
        ...
        $table = Table::getInstance('Design', 'JPtable');
        $data  = array('project_id' => $this->project_id, 'album_id' => $this->album_id, 'alias' => $this->alias);

        if ($table->load($data) && ($table->id != $this->id || $this->id == 0)) {
            $this->setError(Text::_('COM_JOOMPROJECT_ERROR_DESIGN_UNIQUE_ALIAS'));
            return false;
        }
    ...
    public function deleteReferences($pk = null)
    {
        ...
        $query->delete('#__jp_ref_observer')
              ...
              ->where('item_type = ' . $this->_db->quote('com_jpdesigns.design'));
        ...
        // Delete approvals
        $query->clear();
        $query->delete('#__jp_designs_approved')
        ...
        $query->clear();
        $query->select('file_name, project_id')
        ...
        if ($media) {
            $model = BaseDatabaseModel::getInstance('Design', 'JPdesignsModel', array('ignore_request' => true));
            $model->deleteFile($media->file_name, $media->project_id, $pk);
        }
