...
if ($save_order) {
    $order_url = 'index.php?option=com_jpdesigns&task=albums.saveOrderAjax&tmpl=component';
    HTMLHelper::_('sortablelist.sortable', 'albumList', 'adminForm', strtolower($list_dir), $order_url);
}
...
<form action="<?php echo Route::_('index.php?option=com_jpdesigns&view=albums'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="row">
        ...
        <div id="j-main-container" class="col-md-10">
            ...
            <table class="adminlist table table-striped" id="albumList">
                <thead>
                <tr>
                    ...
                    <th width="5%">
                        <?php echo HTMLHelper::_('searchtools.sort', 'JGRID_HEADING_DESIGNS', 'design_count', $list_dir, $list_order); ?>
                    </th>
                ...
                <?php foreach ($this->items as $i => $item) :
                    ...
                    if ($prev_item) {
                        $order_up = ($item->project_id == $prev_item->project_id);
                    }

                    if ($next_item) {
                        $order_down = ($item->project_id == $next_item->project_id);
                    }
                    ?>
                    ...
                    <tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->project_id; ?>">
                        ...
                        <td class="center">
                            <?php echo HTMLHelper::_('jgrid.published', $item->state, $i, 'albums.', $can_change, 'cb'); ?>
                        </td>
                        <td>
                            ...
                            <?php if ($can_edit || $can_edit_own) : ?>
                                <a href="<?php echo Route::_('index.php?option=com_jpdesigns&task=album.edit&id=' . $item->id);?>">
                        ...
                        <td class="center">
                            <?php if ($item->design_count == 0) : ?>
                                <?php if ($can_create) : ?>
                                    <a href="<?php echo Route::_('index.php?option=com_jpdesigns&view=designs&filter_project=' . $item->project_id . '&filter_album=' . $item->id . '&task=design.add');?>">
                                        0
                                    </a>
                                <?php else : ?>
                                    0
                                <?php endif; ?>
                            <?php else : ?>
                                <a href="<?php echo Route::_('index.php?option=com_jpdesigns&view=designs&filter_project=' . $item->project_id . '&filter_album=' . $item->id);?>">
                                    <?php echo (int) $item->design_count; ?>
                                </a>
                            <?php endif; ?>
                        </td>
                        <?php if (!$this->state->get('filter.project')) : ?>
