...
<form action="<?php echo Route::_('index.php?option=com_jpdesigns&view=design&id=' . (int)$this->item->id); ?>"
    ...
    <?php echo HTMLHelper::_('uitab.addTab', 'myTab', 'general', '<i class="fas fa-eye"></i> ' . Text::_('COM_JOOMPROJECT_NEW_DESIGN')); ?>
    <fieldset class="adminform">
        <legend>
            <?php if (empty($this->item->id)) : ?>
                <?php echo Text::_('COM_JOOMPROJECT_NEW_DESIGN'); ?>
            <?php else : ?>
                <?php echo Text::_('COM_JOOMPROJECT_EDIT_DESIGN'); ?>
            <?php endif; ?>
        </legend>
        <?php echo $this->form->renderField('project_id') ?>
        <?php echo $this->form->renderField('file'); ?>
        <?php if ($params->get('enable_custom_thumbnail',0)): ?>
