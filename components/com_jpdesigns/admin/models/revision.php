...
/**
 * Item Model for a design revision form.
 *
 */
class JPdesignsModelRevision extends AdminModel
{
    /**
     ...
     */
    public function getTable($type = 'Revision', $prefix = 'JPtable', $config = array())
    {
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_jpdesigns.revision', 'revision', array('control' => 'jform', 'load_data' => $loadData));
        ...
        if ($id != 0 && (!$user->authorise('core.edit.state', 'com_jpdesigns.revision.' . $id)) || ($id == 0 && !$user->authorise('core.edit.state', 'com_jpdesigns')))
        {
        ...
        if ($id) {
            ...
            if (!isset($data['parent_id'])) {
                ...
                $db->setQuery($query);
                $form->setValue('parent_id', null, (int) $db->loadResult());
            }
        }
        else {
    ...
    protected function populateState()
    {
        ...
        if ($pk) {
            ...
            elseif ($parent) {
                $table = $this->getTable('Design');

                if ($table->load($parent)) {
                    $project = (int) $table->project_id;
    ...
    public function upload($file = NULL, $project = 0, $stream = false)
    {
        ...
        if (!in_array($ext, array('jpeg', 'jpg', 'png', 'gif'))) {
            $this->setError(Text::_('COM_JPDESIGNS_WARNING_FILE_TYPE_NOT_SUPPORTED'));
            return false;
        }
        ...
        if (\Joomla\CMS\Filesystem\File::upload($file['tmp_name'], $uploadpath . '/' . $name, $stream) === true) {
    ...
    public function deleteFile($name, $project = 0, $id = 0)
    {
        ...
        if (\Joomla\CMS\Filesystem\File::exists($uploadpath . '/' . $name)) {
            if (\Joomla\CMS\Filesystem\File::delete($uploadpath . '/' . $name) !== true) {
                return false;
            }
            else {
                // Delete the cache files too
                $size_preview = ComponentHelper::getParams('com_jpdesigns', true)->get('img_preview_size', '300x200');
                $size_full    = ComponentHelper::getParams('com_jpdesigns', true)->get('img_full_size', '1024x768');

                list($width_preview, $height_preview) = explode('x', $size_preview);
                list($width_full,    $height_full)    = explode('x', $size_full);

                $file_preview = md5('revision:' . $project . ':' . $id . ':' . $width_preview . ':' . $height_preview) . '.jpg';
                $file_full    = md5('revision:' . $project . ':' . $id . ':' . $width_full . ':' . $height_full) . '.jpg';

                $base =  JPPATH_CACHE;
                if (\Joomla\CMS\Filesystem\File::exists($base . '/com_jpdesigns.images/' . $file_preview)) {
                    \Joomla\CMS\Filesystem\File::delete($base . '/com_jpdesigns.images/' . $file_preview);
                }

                if (\Joomla\CMS\Filesystem\File::exists($base . '/com_jpdesigns.images/' . $file_full)) {
                    \Joomla\CMS\Filesystem\File::delete($base . '/com_jpdesigns.images/' . $file_full);
                }
                return true;
            }
        }
    ...
    public function save($data)
    {
        ...
        try {
            ...
            if ($data['title'] == '' && isset($data['file']['name'])) {
                $data['title'] = \Joomla\CMS\Filesystem\File::stripExt($data['file']['name']);
            }
            ...
            if (isset($data['labels'])) {
                ...
                $labels->setState('item.type', 'com_jpdesigns.revision');
    ...
    /**
     * Method to set the approval state of a revision record
     *
     * @param     integer    $id       The revision id
     * @param     integer    $parent   The design id
     * @param     integer    $id       The user id
     * @param     integer    $state    The approval state
     *
     * @return    boolean              True on success, False on error
     */
    public function approve($id, $parent, $user_id, $state = 0)
    {
        $db    = Factory::getDbo();
        $query = $db->getQuery(true);
        ...
        $query->select('created')
              ...
              ->where('revision_id = ' . $db->quote((int) $id))
    ...
    public function import($data)
    {
        ...
        if (!\Joomla\CMS\Filesystem\File::exists($source_path)) {
        ...
        if (!\Joomla\CMS\Filesystem\File::copy($source_path, $base_path . '/' . $file_name)) {
        ...
        $item['file']['extension'] = strtolower(\Joomla\CMS\Filesystem\File::getExt($file_name));

        if (!$this->save($item)) {
            \Joomla\CMS\Filesystem\File::delete($source_path, $base_path . '/' . $file_name);
            return false;
        }

        if (!\Joomla\CMS\Filesystem\File::delete($source_path)) {
    ...
    public function watch(&$pks, $value = 1, $uid = null)
    {
        ...
        $item_type = 'com_jpdesigns.revision';
    ...
    {
        parent::cleanCache('com_jpdesigns');
    }
    ...
    {
        $name = \Joomla\CMS\Filesystem\File::makeSafe($name);
        $ext  = strtolower(\Joomla\CMS\Filesystem\File::getExt($name));
        $name = substr($name, 0 , (strlen($name) - (strlen($ext) + 1)));

        if ($name == '') {
            $name = \Joomla\CMS\Filesystem\File::makeSafe(Factory::getDate()->format('Y-m-d-H-i-s'));
        }
        ...
        while ($exists == true)
        {
            ...
            if (preg_match('#-(\d+)$#', $name, $m)) {
                ...
                $exists = \Joomla\CMS\Filesystem\File::exists($dest . '/' . $name . '.' . $ext);
            }
            else {
                $name  .= '-2';
                $exists = \Joomla\CMS\Filesystem\File::exists($dest . '/' . $name . '.' . $ext);
     ...
     * @param     integer    The parent design id
     * @param     integer    The item id
     *
     * @return    array      Contains the modified title and alias
     */
    protected function generateNewTitle($title, $project, $parent = 0, $id = 0)
    {
        $table = $this->getTable();
        $db    = Factory::getDbo();
        $query = $db->getQuery(true);
        ...
        elseif ($id == 0 && $count == 0) {
            ...
            while ($table->load(array('parent_id' => $parent, 'project_id' => $project, 'alias' => $alias)))
            {
                $m = null;
    ...
    protected function canDelete($record)
    {
        if (!empty($record->id)) {
            ...
            $user  = Factory::getApplication()->getIdentity();
            $asset = 'com_jpdesigns.revision.' . (int) $record->id;
    ...
    protected function loadFormData()
    {
        ...
        if (empty($data)) {
            ...
            if ($this->getState($this->getName() . '.id') == 0) {
                ...
                $data->set('parent_id', (int) $this->getState($this->getName() . '.parent_id'));
            }
        }
