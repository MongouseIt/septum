...
/**
 * Methods supporting a list of design records.
 *
 */
class JPdesignsModelDesigns extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        $query->select(
            $this->getState(
                'list.select',
                'a.id, a.project_id, a.album_id, a.title, a.alias, a.checked_out, a.checked_out_time,'
        ...
        if ((int) $this->getState('filter.parent_id') <= 0) {
            $query->select('COUNT(DISTINCT b.id) AS revision_count')
                  ->join('LEFT', '#__jp_design_revisions AS b ON b.parent_id = a.id');
        }
        ...
        if ($album = $this->getState('filter.album')) {
            $query->where('a.album_id = ' . (int) $album);
        }
        ...
        if ($order_col == 'a.ordering' || $order_col == 'category_title') {
			$order_col = 'a.project_id, a.album_id ' . $order_dir . ', a.ordering';
		}
