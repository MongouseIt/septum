...
namespace Joomla\Component\JPdesigns\Site\Service;
...
class FormRules implements RulesInterface
{
    ...
    public function build(&$query, &$segments)
    {
        ...
        $segments = self::JPdesignsBuildRoute($query);
    ...
    {
        $vars = self::JPdesignsParseRoute($segments);
    ...
    /**
     * Build the route for the com_jpdesigns component
     *
     * @param array $query An array of URL arguments
     *
     * @return    array              The URL arguments to use to assemble the subsequent URL.
     */
    public static function JPdesignsBuildRoute(&$query)
    {

        $menu = \Joomla\CMS\Factory::getContainer()->get(SiteApplication::class)->getMenu();

        $menuItem = isset($query['Itemid']) ? $menu->getItem($query['Itemid']) : null;
        ...
        if ($view == 'albums') {
            ...
            if (isset($query['filter_project'])) {
                if (strpos($query['filter_project'], ':') === false) {
                    $query['filter_project'] = self::JPdesignsMakeSlug($query['filter_project'], '#__jp_projects');
                }
            } else {
                $query['filter_project'] = self::JPdesignsMakeSlug('0', '#__jp_projects');
            }
        ...
        // Handle designs query
        if ($view == 'designs') {

            if (
                (isset($menuItem->query['view']) && $menuItem->query['view'] != $query['view']) || // current item id view not equal to query view
                !$menu_item_given // or no given item id
            ) {
                $segments[] = $view; // then keep view
            }
            ...
            // Get category filter
            if (isset($query['filter_album'])) {
                if (strpos($query['filter_album'], ':') === false) {
                    $query['filter_album'] = self::JPdesignsMakeSlug($query['filter_album'], '#__jp_design_albums', 'all-albums');
                }
            } else {
                $query['filter_album'] = self::JPdesignsMakeSlug('0', '#__jp_design_albums', 'all-albums');
            }

            $segments[] = $query['filter_album'];
            unset($query['filter_album']);
        ...
        if ($view == 'design') {
            ...
            if (isset($query['filter_album'])) {
                if (strpos($query['filter_album'], ':') === false) {
                    $query['filter_album'] = self::JPdesignsMakeSlug($query['filter_album'], '#__jp_design_albums', 'no-album');
                }
            } else {
                $query['filter_album'] = self::JPdesignsMakeSlug('0', '#__jp_design_albums', 'no-album');
            }
            ...
            if (isset($query['id'])) {
                ...
                if (strpos($query['id'], ':') === false) {
                    $query['id'] = self::JPdesignsMakeSlug($query['id'], '#__jp_designs');
                }
                ...
                // Get revision filter
                if (strpos($query['revision'], ':') === false) {
                    $query['revision'] = self::JPdesignsMakeSlug($query['revision'], '#__jp_design_revisions', 'original');
                }

                $segments[] = $query['revision'];
     ...
     */
    public static function JPdesignsParseRoute($segments)
    {
        ...
        $menu = \Joomla\CMS\Factory::getContainer()->get(SiteApplication::class)->getMenu();
        $item = $menu->getActive();
        $viewsSupportPrerocess = ['design'];
        $viewToPrerocess= in_array($segments[0],$viewsSupportPrerocess) ? $segments[0] : '';
        ...
        if(!self::preProcessParse($vars,$segments,$item,$viewToPrerocess))
            return $vars;


        // Handle album overview
        if ($vars['view'] == 'albums') {
            if ($count >= 1) {
                $vars['filter_project'] = self::JPdesignsParseSlug($segments[0]);
            }
            if ($count >= 2) {
                $vars['view'] = 'designs';
            } else {
        ...
        // Handle design overview
        if ($vars['view'] == 'designs') {

            if ($count >= 1) {
                $vars['filter_project'] = self::JPdesignsParseSlug($segments[0]);
            }
            if ($count >= 2) {
                $vars['filter_album'] = self::JPdesignsParseSlug($segments[1]);
            }
            if ($count >= 3) {
                $vars['id'] = self::JPdesignsParseSlug($segments[2]);
                $vars['view'] = 'design';
            }
            if ($count >= 4) {
                $vars['revision'] = self::JPdesignsParseSlug($segments[3]);
            }
    ...
    public static function preProcessParse(&$vars, &$segments, $item, $view = '')
    {
        ...
        // given view
        if (!isset($item) && $segments[0] == $view) { // if design has no menu item
            $vars['view'] = $view;
            array_splice($segments, 1, 1);

        }elseif(isset($item) && $segments[0] == $view) { // if design has menu item
            $vars['view'] = $view;
            array_splice($segments, 1, 1);
        }
     ...
     */
    public static function JPdesignsParseSlug($segment)
    {
     ...
     */
    public static function JPdesignsMakeSlug($id, $table, $alt = 'all', $field = 'alias')
    {
        if ($id == '' || $id == '0') {
