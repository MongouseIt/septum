 ...
 *
 */
class JPdesignsControllerDesignForm extends FormController
{
    /**
     * Default item view
     *
     * @var    string
     */
    protected $view_item = 'designform';
    ...
    public function __construct($config = array())
	{
        ...
        $this->registerTask('save2rev', 'save');
    ...
    public function save($key = null, $urlVar = null)
    {
        ...
        if (!$this->checkEditId($context, $record_id)) {
			...
			$this->setMessage(Text::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $record_id));
        ...
        if ($task == 'upload') {
            if (!isset($data['project_id'])) {
                ...
                $album_model = $this->getModel('AlbumForm');
                $album = $album_model->getItem((int) $data['album_id']);

                if ($album->getError()) {
                    $this->setMessage($album->getError());
                ...
                $data['project_id'] = (int) $album->project_id;
        ...
        if(
            ...
            !empty($files['thumbnail']['tmp_name'])
        ){
    ...
    public function approve($key = null, $urlVar = null)
    {
        ...
        if (!$this->allowApprove($record_id)) {
            // Access denied
        ...
        if (!$model->approve($record_id, $user->get('id'), $state)) {
        ...
        $text = ($state == 1 ? Text::_('COM_JOOMPROJECT_DESIGN_ITEM_APPROVED') : Text::_('COM_JOOMPROJECT_DESIGN_ITEM_DECLINED'));
    ...
    protected function postSaveHook($model, $validData = [])
    {
        ...
        switch($task)
        {
            ...
            case 'save2rev':
                $link = Route::_(JPdesignsHelperRoute::getDesignsRoute() . '&task=revisionform.add', false);
                $this->setRedirect($link);
                break;

            default:
                $this->setRedirect($this->getReturnPage() . ($task == 'upload' ? '#_' . $validData['album_id'] : ''));
                break;
        }
    }
}
