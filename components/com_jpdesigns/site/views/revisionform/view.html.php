...
/**
 * Revision Form View Class for Joomproject Designs component
 *
 */
class JPdesignsViewRevisionForm extends HtmlView
{
    protected $form;
    protected $item;
    ...
    public function display($tpl = null)
    {
        ...
        // Permission check.
        if ($this->item->id <= 0) {
            $access = JPdesignsHelper::getRevisionActions(0, (int) $this->state->get('revisionform.parent_id'));
    ...
    protected function _prepareDocument()
    {
        ...
        $def_title = Text::_('COM_JOOMPROJECT_PAGE_' . ($this->item->id > 0 ? 'EDIT' : 'ADD') . '_DESIGN_REVISION');

        // Because the application sets a default page title, we need to get it from the menu item itself
        if ($menu) {
            if (strpos($menu->link, 'view=designs') !== false) {
    ...
    protected function getToolbar()
    {
        ...
        JPToolbar::dropdownButton($options, array('class' => 'btn-success',  'icon' => 'fas fa-ok'));
