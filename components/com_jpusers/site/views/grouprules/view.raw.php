...
class JPusersViewGroupRules extends HtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        /// currently check owner permission only jpprojects
        $this->itemTypeData = $this->component == 'com_jpprojects' ? JPprojectsHelperProject::getInfo($this->project_id) : null;
        $this->itemTypeId = $this->component == 'com_jpprojects' ? $this->project_id : null;


        if (
            !JoomprojectHelperAccess::canChangePermissions($this->itemTypeId,$this->itemTypeData,$user,$this->component)
        ) {
    ...
    protected function getAssetRules($component = null, $asset_id = null)
    {
        ...
        $cache[$asset_id] = Access::getAssetRules($asset_id,false, true, false);
    ...
    protected function getCalculated($action, $rule, $calc)
    {
        ...
        elseif (!empty($this->component)) {
            ...
            elseif ($calc === false) {
                ...
                $html = '<span class="fas fa-ban text-danger"><span class="fas fa-lock">' . Text::_('JLIB_RULES_NOT_ALLOWED_ADMIN_CONFLICT') . '</span></span>';
            }
            else {
                $html = '<span class="fas fa-check text-success"><span class="fas fa-lock">' . Text::_('JLIB_RULES_ALLOWED_ADMIN') . '</span></span>';
            }
