 ...
 *
 */
class JFormFieldUserRef extends UserField
{
    /**
     * The form field type.
     *
    ...
    protected function getInput()
    {
        ...
        if (!empty($this->value) && is_array($this->value)) {
            ...
            if (count($this->value)) {
                ArrayHelper::toInteger($this->value);
    ...
    protected function getSelect2HTML($value, $multiple = false)
    {
        ...
        $pks    = \Joomla\Utilities\ArrayHelper::getColumn((array) $value, 'id');
        $html   = array();

        if (!$access) {
            $access = (int) Factory::getConfig()->get('access');
        }

        $url = Uri::base()."index.php?option=com_jpusers&view=userref&filter_access=" . $access . "&tmpl=component&layout=select2&format=json";
        ...
        if ($multiple) {
            ...
            foreach ($value AS $i => $user)
            {
                ...
                if ($this->element['readonly'] != 'true') {
                    ...
                    $html[] = '        <a class="modal_' . $this->id . '" title="' . Text::_('JLIB_FORM_CHANGE_USER') . '"' . ' href="' . $link . '"'
                ...
                if ($this->element['readonly'] != 'true') {
                    ...
                    $html[] = '        ' . Text::_('COM_JOOMPROJECT_FIELD_CLEAR_LABEL') . '</a>';
                    $html[] = '  </div>';
                    $html[] = '</div>';
                }
            ...
            $js_clear = 'document.getElementById(\'' . $this->id.'_name\').value = \'\';'
    ...
    protected function getSiteHTML($value, $multiple = false)
    {
        ...
        $link = JPusersHelperRoute::getUsersRoute() . '&amp;layout=modal&amp;tmpl=component&amp;field=' . $this->id
              . (isset($groups) ? ('&amp;groups=' . base64_encode(json_encode($groups)))       : '')
              . (isset($excluded) ? ('&amp;excluded=' . base64_encode(json_encode($excluded))) : '');
        ...
        if ($multiple) {
            ...
            foreach ($value AS $i => $user)
            {
                ...
                // Create the user select button.
                if ($this->element['readonly'] != 'true') {
                    $html[] = '<a class="modal_' . $this->id . ' btn" title="' . Text::_('JLIB_FORM_CHANGE_USER') . '"' . ' href="' . $link . '"'
                            . ' rel="{handler: \'iframe\', size: {x: 800, y: 500}}" onclick="jQuery(\'#' . $this->id . '_i\').val(' . $i . ');">'
                ...
                if ($this->element['readonly'] != 'true') {
                    $html[] = '<a onclick="jRemoveUserField_' . $this->id . '(this)" class="btn">'
                            . Text::_('COM_JOOMPROJECT_FIELD_CLEAR_LABEL')
                            . '</a>';
                }
    ...
    protected function getGroups()
    {
        ...
        $groups     = array();
        $group_list = (array) JPAccessHelper::getGroupsByAccessLevel($access);
    ...
    protected function getJavascript($onchange = '', $multiple = false)
    {
        ...
        if ($multiple) {
            if (Factory::getApplication()->isClient('site')) {
                ...
                $script[] = '    var btn = "<a class=\"modal_' . $this->id . '_" + i + " btn\" title=\"' . Text::_('JLIB_FORM_CHANGE_USER') . '\"' . ' href=\"' . $link . '\""';
                $script[] = '            + " rel=\"{handler: \'iframe\', size: {x: 800, y: 500}}\" onclick=\"jQuery(\'#' . $this->id . '_i\').val(" + i + ");\">"';
                $script[] = '            + "' . Text::_('JLIB_FORM_CHANGE_USER') . '"';
                $script[] = '            + "</a>"';

                $script[] = '    var clr = "<a onclick=\"jRemoveUserField_' . $this->id . '(this)\" class=\"btn\">"';
                $script[] = '            + "' . Text::_('COM_JOOMPROJECT_FIELD_CLEAR_LABEL') . '"';
                $script[] = '            + "</a>"';
               ...
               // $script[] = '    SqueezeBox.initialize({});SqueezeBox.assign($$("a.modal_' . $this->id . '_" + i), {parse: "rel"});';
                ...
                $script[] = '            + "<a class=\"modal_' . $this->id . '_" + i + " btn\" title=\"' . Text::_('JLIB_FORM_CHANGE_USER') . '\"' . ' href=\"' . $link . '\""';
                $script[] = '            + " rel=\"{handler: \'iframe\', size: {x: 800, y: 500}}\" onclick=\"jQuery(\'#' . $this->id . '_i\').val(" + i + ");\">"';
                $script[] = '            + "' . Text::_('JLIB_FORM_CHANGE_USER') . '"';
                $script[] = '            + "</a>"';
                $script[] = '            + "</div></div>"';
