...
namespace Joomla\Component\JPmilestones\Site\Service;
...
class FormRules implements RulesInterface
{
    ...
    {
        $segments = self::JPmilestonesBuildRoute($query);
    ...
    {
        $vars = self::JPmilestonesParseRoute($segments);
    ...
    /**
     * Build the route for the com_jpmilestones component
     *
     * @param     array    $query    An array of URL arguments
     *
     * @return    array              The URL arguments to use to assemble the subsequent URL.
     */
    public static function JPmilestonesBuildRoute(&$query)
    {
        ...
        if($view == 'milestones' || $view == 'milestone') {
            ...
            if (isset($query['filter_project'])) {
                if (strpos($query['filter_project'], ':') === false) {
                    $query['filter_project'] = self::JPmilestonesMakeSlug($query['filter_project'], '#__jp_projects');
                }
            }
            else {
                $query['filter_project'] = self::JPmilestonesMakeSlug('0', '#__jp_projects');
            }
            ...
            if ($view == 'milestone' && isset($query['id'])) {
                if (strpos($query['id'], ':') === false) {
                    $query['id'] = self::JPmilestonesMakeSlug($query['id'], '#__jp_milestones');
                }
     ...
     */
    public static function JPmilestonesParseRoute($segments)
    {
        ...
        if ($vars['view'] == 'milestones') {
            if ($count >= 1) {
                $vars['filter_project'] = self::JPmilestonesParseSlug($segments[0]);
            }
            if ($count >= 2) {
                $vars['view'] = 'milestone';
                $vars['id']   = self::JPmilestonesParseSlug($segments[1]);
            }
     ...
     */
    public static function JPmilestonesParseSlug($segment)
    {
     ...
     */
    public static function JPmilestonesMakeSlug($id, $table, $alt = 'all', $field = 'alias')
    {
        if ($id == '' || $id == '0') {
