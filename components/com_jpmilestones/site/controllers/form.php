...
/**
 * Joomproject Milestone Form Controller
 *
 */
class JPmilestonesControllerForm extends FormController
{
    /**
    ...
    protected function allowEdit($data = array(), $key = 'id')
    {
        ...
        $user  = Factory::getApplication()->getIdentity();
        $uid   = $user->get('id');
        $asset = 'com_jpmilestones.milestone.' . $id;
    ...
    protected function getReturnPage()
    {
        ...
        if (empty($return) || !Uri::isInternal(base64_decode($return))) {
            return Route::_('index.php?option=com_jpmilestones&view=' . $this->view_list, false);
        }
        else {
    ...
    protected function postSaveHook(BaseDatabaseModel $model, $data = array())
    {
        ...
        switch($task)
        {
            ...
            case 'save2tasklist':
                $link = Route::_(JPtasksHelperRoute::getTasksRoute() . '&task=tasklistform.add&milestone_id=' . $id);
                $this->setRedirect($link);
                break;

            case 'save2task':
                $link = Route::_(JPtasksHelperRoute::getTasksRoute() . '&task=taskform.add&milestone_id=' . $id);
                $this->setRedirect($link);
                break;
