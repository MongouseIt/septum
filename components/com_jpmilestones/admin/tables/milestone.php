...
JLoader::register('JPtableMilestoneCompat', dirname(__FILE__) . '/milestone_compat.php');
 ...
 *
 */
class JPtableMilestone extends JPtableMilestoneCompat
{
    /**
     * Constructor
     *
    ...
    {
        parent::__construct('#__jp_milestones', 'id', $db);
    ...
    {
        $k = $this->_tbl_key;
        return 'com_jpmilestones.milestone.' . (int) $this->$k;
    }
    ...
    {
        if ((int) $this->project_id == 0) return (int) Factory::getConfig()->get('access');
    ...
    protected function _getAssetParentIdCompat($table = null, $id = null)
    {
        ...
        $query->select('id')
              ->from('#__assets')
              ->where('name' . ' = ' . $this->_db->quote("com_jpmilestones.project." . (int) $this->project_id));
        ...
        if (!$result) {
            ...
            $query->select('id')
                  ->from('#__assets')
                  ->where('name' . ' = ' . $this->_db->quote("com_jpmilestones"));
    ...
    public function check()
    {
        ...
        // clone end date to be used on timeline automation in jptasks plugin
        $this->end_date_request = $this->end_date;

        if (trim($this->alias) == '') $this->alias = $this->title;
        $this->alias = Joomla\CMS\Application\ApplicationHelper::stringURLSafe($this->alias);
        if (trim(str_replace('-','', $this->alias)) == '') $this->alias = Factory::getDate()->format('Y-m-d-H-i-s');
        ...
        // if batch copy operation , we need to shift dates
        if($task == 'batch'){
    ...
    public function store($updateNulls = false)
    {
        ...
        if ($table->load(array('alias' => $this->alias, 'project_id' => $this->project_id)) && ($table->id != $this->id || $this->id==0)) {
            $this->setError(Text::_('JLIB_DATABASE_ERROR_MILESTONE_UNIQUE_ALIAS'));
            return false;
        }
    ...
    public function deleteReferences($pk = null)
    {
        ...
        $query->delete('#__jp_ref_attachments')
              ...
              ->where('item_type = ' . $this->_db->quote('com_jpmilestones.milestone'));
