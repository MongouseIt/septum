 ...
 *
 */
class JPtasksModelTasklists extends ListModel
{
    /**
     * Constructor
     *
    ...
    public function getAuthors()
    {
        ...
        $query->select('u.id AS value, u.name AS text')
              ->from('#__users AS u')
              ->join('INNER', '#__jp_task_lists AS a ON a.created_by = u.id')
    ...
    public function getMilestones()
    {
        ...
        $query->select('m.id AS value, m.title AS text')
              ...
              ->join('INNER', '#__jp_task_lists AS a ON a.milestone_id = m.id')
              ->where('a.project_id = ' . $this->_db->quote($project))
              ->group('m.id')
              ->order('m.title');
    ...
    protected function getListQuery()
    {
        ...
        $query->select('COUNT(t.id) AS task_count')
              ->join('LEFT', '#__jp_tasks AS t ON t.list_id = a.id');
