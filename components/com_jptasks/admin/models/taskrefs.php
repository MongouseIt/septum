 ...
 *
 */
class JPtasksModelTaskRefs extends ListModel
{
    /**
     * Constructor
     *
    ...
    public function getItems($task_id = 0, $diagnostic = false)
    {
        ...
        if (empty($task_id)) {
            $this->setError(Text::_('COM_JOOMPROJECT_WARNING_DEPENDENCY_NO_ITEM_REFERENCE'));
            return array();
        }
        ...
        $query->select($fields)
              ->from('#__jp_ref_tasks AS a')
              ->where('a.task_id = ' . $db->quote((int) $task_id));

        if (!$diagnostic) {
            $query->join('left', '#__jp_tasks AS t ON (t.id = a.parent_id)')
                  ->group('a.parent_id');
        }
    ...
    public function save($data = array(), $task_id = 0, $project_id = 0)
    {
        ...
        $taskref  = $this->getInstance('TaskRef', 'JPtasksModel', array('ignore_request' => true));
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        // Project id
        $value = (int) $this->getUserStateFromRequest('com_joomproject.project.active.id', 'filter_project', '');
