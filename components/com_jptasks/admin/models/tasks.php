...
class JPtasksModelTasks extends ListModel
{
    ...
    public function getAuthors()
    {
        ...
        $query->select('u.id AS value, u.name AS text')
              ->from('#__users AS u')
              ->join('INNER', '#__jp_tasks AS a ON a.created_by = u.id')
    ...
    protected function getListQuery()
    {
        ...
        $query->select('tl.title AS tasklist_title')
              ->join('LEFT', '#__jp_task_lists AS tl ON tl.id = a.list_id');
        ...
        if ($published !== '*')
        {
            ...
            {
                $state = (int) $published;
                $query->where($this->_db->quoteName('a.state') . ' = :state')
                    ->bind(':state', $state, ParameterType::INTEGER);
            }
            else
            {
                $query->whereIn(
                    $this->_db->quoteName('a.state'),
                    [
        ...
        // Filter by milestone
        if (is_numeric($filter_ms) && $filter_ms > 0) {
            $query->where('a.milestone_id = ' . (int) $filter_ms);
        }

        // Filter by task list
        if (is_numeric($filter_list) && $filter_list > 0) {
            $query->where('a.list_id = ' . (int) $filter_list);
        }


        // Filter by priority
        if (is_numeric($filter_priority) && $filter_priority > 0) {
            $query->where('a.priority = ' . (int) $filter_priority);
        }

        // Filter by completition
        if (is_numeric($filter_complete) && $filter_complete > 0) {
