 ...
 *
 */
class JPactivitiesModelItem extends AdminModel
{
    /**
     ...
     */
    public function &getTable($type = 'Item', $prefix = 'JPactivitiesTable', $config = array())
    {
        static $table = array();
        ...
        if (!isset($table[$key])) {
            $table[$key] = Table::getInstance($type, $prefix, $config);
        }
    ...
    public function getType($extension, $name, $plugin = null)
    {
        ...
        $cache[$key] = $db->loadResult();

        if (is_null($cache[$key]) && $plugin) {
            ...
            $result = $db->insertObject('#__user_activity_item_types', $obj);
             ...
             */
             if (!isset($data['vaccess'])) {
                if (isset($data['access'])) {
                    ...
                    $data['vaccess'] = Factory::getConfig()->get('access');
                }
             }
    ...
    protected function getAssetId($name, $title = null, $create = false)
    {
        ...
        if (!isset($parents[$name])) {
            ...
            $query->clear()
                  ->select('id')
                  ->from('#__assets')
                  ->where('name = ' . $this->_db->quote('com_jpactivities'));
        ...
        $asset = Table::getInstance('Asset', 'JTable', array('dbo' => $this->_db));
