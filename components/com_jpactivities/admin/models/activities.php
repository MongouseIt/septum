...
JPactivitiesHelper::loadPluginLanguages();
 ...
 *
 */
class JPactivitiesModelActivities extends ListModel
{
    /**
    ...
    {
        $params = ComponentHelper::getParams('com_jpactivities');
    ...
    public function getItems()
    {
        ...
        if (JDEBUG) Profiler::getInstance('Application')->mark('beforeJPactivitiesGetItems');
        ...
        // Translate items
        $count   = count($items);
        $clients = array('0' => Text::_('JSITE'), '1' => Text::_('JADMINISTRATOR'));
        ...
        foreach ($items AS &$item)
        {
            ...
            $item = JPactivitiesHelper::translate($item, $config);
        }

        // Add the items to the internal cache.
        $this->cache[$store] = $items;

        if (JDEBUG) Profiler::getInstance('Application')->mark('afterJPactivitiesGetItems');
    ...
    {
        if (JDEBUG) Profiler::getInstance('Application')->mark('beforeJPactivitiesGetItemsGrouped');
        ...
        // Translate the items
        $clients = array(Text::_('JSITE'), Text::_('JADMINISTRATOR'));
        ...
        foreach ($items AS &$item)
        {
            ...
            if (count($item) == 1) {
                $key  = key($item);
                $item = JPactivitiesHelper::translate($item[$key], $config);
            }
            else {
                $item = JPactivitiesHelper::translate($item, $config);
            }
        ...
        if (JDEBUG) Profiler::getInstance('Application')->mark('afterJPactivitiesGetItemsGrouped');
    ...
    public function getLocations()
    {
        ...
        $items[0]->value = 0;
        $items[0]->text  = Text::_('JSITE');

        $items[1]->value = 1;
        $items[1]->text  = Text::_('JADMINISTRATOR');
    ...
    protected function getListQuery()
    {
        ...
        // Exclude disabled extensions on frontend
        if(Factory::getApplication()->isClient('site')) {
            GlobalAccess::allowedMainComponents();
            $disabledExtensions = GlobalAccess::$notAllowedComponents;

            if(!empty($disabledExtensions)){

                $disabledExtensions = implode(',', $this->_db->quote($disabledExtensions));
                $query->where('t.extension NOT IN('.$disabledExtensions.')');
            }
        }
