 ...
 *
 */
class JPactivitiesHelper
{
    public static $extension = 'com_jpactivities';
     ...
     */
    public static function getPluginHelper($name, $type = 'jpactivities', $config = array())
    {
        ...
        $helper_file = Path::check(JPATH_PLUGINS . '/' . $type . '/' . $name . '/helpers/' . $name . '.php');
    ...
    public static function relativeDateTime($datetime, $tz = true)
    {
        ...
        if ($tz) {
			...
			$time_date = Factory::getDate($datetime, 'UTC');
        ...
        if ($months) {
            return Text::sprintf('COM_JPACTIVITIES_DT_MONTH' . ($months > 1 ? 'S' : '') . '_AGO', $months);
        }

        if ($days) {
            return Text::sprintf('COM_JPACTIVITIES_DT_DAY' . ($days > 1 ? 'S' : '') . '_AGO', $days);
        }

        if ($hours) {
            return Text::sprintf('COM_JPACTIVITIES_DT_HOUR' . ($hours > 1 ? 'S' : '') . '_AGO', $hours);
        }

        if ($minutes) {
            return Text::sprintf('COM_JPACTIVITIES_DT_MIN' . ($minutes > 1 ? 'S' : '') . '_AGO', $minutes);
        }

        return Text::_('COM_JPACTIVITIES_DT_JUST_NOW');
    }
}
