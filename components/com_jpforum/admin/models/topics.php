...
/**
 * Class supporting a list of topic records.
 *
 */
class JPforumModelTopics extends ListModel
{
    ...
    public function getAuthors()
    {
        ...
        $query->select('u.id AS value, u.name AS text')
              ->from('#__users AS u')
              ->join('INNER', '#__jp_topics AS a ON a.created_by = u.id')
    ...
    protected function getListQuery()
    {
        ...
        $query->select(
            $this->getState(
                'list.select',
                'a.id, a.project_id, a.title, a.alias, a.checked_out, '
        ...
        // Join over the replies for the reply count.
        $query->select('COUNT(r.id) AS replies');
        $query->join('LEFT', '#__jp_replies AS r ON r.topic_id = a.id');
