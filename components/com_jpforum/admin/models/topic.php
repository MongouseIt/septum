 ...
 *
 */
class JPforumModelTopic extends AdminModel
{
    /**
     ...
     */
    public function getTable($type = 'Topic', $prefix = 'JPtable', $config = array())
    {
    ...
    public function getItem($pk = null)
    {
        ...
        if (JPApplicationHelper::exists('com_jprepo')) {
            ...
            $item->attachment = $attachments->getItems('com_jpforum.topic', $item->id);
        }

        // Get the labels
        $model_labels = $this->getInstance('Labels', 'JPModel');
        $item->labels = $model_labels->getConnections('com_jpforum.topic', $item->id);
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_jpforum.topic', 'topic', array('control' => 'jform', 'load_data' => $loadData));
        ...
        if ($id != 0 && (!$user->authorise('core.edit.state', 'com_jpforum.topic.' . $id)) || ($id == 0 && !$user->authorise('core.edit.state', 'com_jpforum')))
        {
        ...
        if (parent::save($data)) {
            ...
            if (isset($data['attachment'])  && JPApplicationHelper::exists('com_jprepo')) {
                ...
                if (!$attachments->getState('item.type')) {
                    $attachments->setState('item.type', 'com_jpforum.topic');
                }
            ...
            if (isset($data['labels'])) {
                ...
                $labels->setState('item.type', 'com_jpforum.topic');
    ...
    protected function canDelete($record)
    {
        ...
        return $user->authorise('core.delete', 'com_jpforum.topic.' . (int) $record->id);
    }
    ...
    protected function canEditState($record)
    {
        ...
        return $user->authorise('core.edit.state', 'com_jpforum.topic.' . (int) $record->id);
    }
    ...
    protected function canEdit($record)
    {
        ...
        $user  = Factory::getApplication()->getIdentity();
        $asset = 'com_jpforum.topic.' . (int) $record->id;
