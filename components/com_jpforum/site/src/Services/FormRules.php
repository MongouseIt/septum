...
namespace Joomla\Component\JPforum\Site\Service;
...
class FormRules implements RulesInterface
{
    ...
    {
        $segments = self::JPforumBuildRoute($query);
    ...
    {
        $vars = self::JPforumParseRoute($segments);
...
/**
* Build the route for the com_jpforum component
*
* @param     array    $query    An array of URL arguments
*
* @return    array              The URL arguments to use to assemble the subsequent URL.
 */
function JPforumBuildRoute(&$query)
{
    ...
    if($view == 'topics') {
        ...
        if (isset($query['filter_project'])) {
            if (strpos($query['filter_project'], ':') === false) {
                $query['filter_project'] = self::JPforumMakeSlug($query['filter_project'], '#__jp_projects');
            }
        }
        else {
            $query['filter_project'] = self::JPforumMakeSlug('0', '#__jp_projects');
        }
    ...
    if($view == 'replies') {
        ...
        if (isset($query['filter_topic'])) {
            if (strpos($query['filter_topic'], ':') === false) {
                $query['filter_topic'] = self::JPforumMakeSlug($query['filter_topic'], '#__jp_topics');
            }
        }
        else {
            $query['filter_topic'] = self::JPforumMakeSlug('0', '#__jp_topics');
        }
 ...
 */
function JPforumParseRoute($segments)
{
    // Setup vars
    $vars  = array();
    ...
    if ($vars['view'] == 'topics') {
        if ($count == 1) {
            $vars['filter_project'] = self::JPforumParseSlug($segments[0]);
        }
        if ($count > 1) {
            $vars['view'] = 'replies';
            $vars['filter_topic'] = self::JPforumParseSlug($segments[1]);
        }
 ...
 */
function JPforumParseSlug($segment)
{
 ...
 */
function JPforumMakeSlug($id, $table, $alt = 'all', $field = 'alias')
{
    if ($id == '' || $id == '0') {
