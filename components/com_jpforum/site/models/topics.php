 ...
 *
 */
class JPforumModelTopics extends ListModel
{
    ...
    public function getListQuery()
    {
        ...
        $query->join('LEFT', '#__jp_replies AS c ON c.topic_id = a.id');

        // Join over the label refs for label count
        $query->select('COUNT(DISTINCT lbl.id) AS label_count');
        $query->join('LEFT', '#__jp_ref_labels AS lbl ON (lbl.item_id = a.id AND lbl.item_type = ' . $db->quote('com_jpforum.topic') . ')');
        ...
        if ($user->get('id') > 0) {
            ...
            $query->join('LEFT', '#__jp_ref_observer AS obs ON (obs.item_type = '
                               . $db->quote('com_jpforum.topic') . ' AND obs.item_id = a.id AND obs.user_id = '
        ...
        $query->join('LEFT', '#__jp_ref_attachments AS at ON (at.item_type = '
              . $db->quote('com_jpforum.topic') . ' AND at.item_id = a.id)');
	    ...
	    JPUserHelper::filterByViewAccess($query,'project','forum');
    ...
    public function getItems()
    {
        ...
        $global_params = ComponentHelper::getParams('com_jpforum', true);
        ...
        foreach ($items as $i => &$item)
        {
            ...
            if ($items[$i]->label_count > 0) {
                $items[$i]->labels = $labels->getConnections('com_jpforum.topic', $items[$i]->id);
            }
        }
    ...
    public function getAuthors()
    {
        ...
        $query  = $db->getQuery(true);
        $user   = Factory::getApplication()->getIdentity();
        $access = JPforumHelper::getActions();
        ...
        $query->join('INNER', '#__jp_topics AS a ON a.created_by = u.id');

        // Implement View Level Access
        if (!$user->authorise('core.admin', 'com_jpforum')) {
    ...
    protected function populateState($ordering = 'last_activity', $direction = 'DESC')
    {
        ...
        // Filter - Search
        $value = \Joomla\CMS\Factory::getApplication()->input->getString('filter_search', '');
