...
class JPforumViewReplies extends HtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        // check global access
        if(!GlobalAccess::check('com_jpforum'))
            return;
        ...
        if ($this->topic === false) {
            Factory::getApplication()->enqueueMessage( $this->topic->getError(),'error');
            return false;
        }
        ...
        $this->access     = JPforumHelper::getActions($this->topic->id);
        ...
        $dispatcher->triggerEvent('onContentPrepare', array('com_jpforum.topic', &$this->topic, &$this->params, 0));
    ...
    protected function prepareDocument()
    {
        ...
        if ($menu) {
            ...
            $this->params->def('page_heading', Text::_('COM_JOOMPROJECT_REPLIES'));
        }
    ...
    {
        $access = JPforumHelper::getActions($this->topic->id);
