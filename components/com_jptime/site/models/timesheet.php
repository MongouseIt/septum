 ...
 *
 */
class JPtimeModelTimesheet extends ListModel
{
	...
	protected function populateState($ordering = 'a.log_date', $direction = 'DESC')
	{
		...
		$input  = $app->input;
		$access = JPtimeHelper::getActions();
		...
		$value = $app->input->get('limit', $params->get('display_num', Factory::getConfig()->get('list_limit')), 'uint');
		...
		// Filter - Date from to
		$start_date = $input->getString('filter_start_date', '');
		$end_date   = $input->getString('filter_end_date', '');
		...
		else
		{
			...
			$this->setState('filter.start_date', null);
		}
		...
		{
			$this->setState('filter.end_date', null);
		}

		// filter by predefined date range
		$daterange = $input->getInt('filter_date_range_type', 0);

		$pdatetype = ($daterange == 0) ?  $params->get('predefined_date_range', 0) : $daterange;
		$this->setState('filter.date_range_type', $pdatetype);
	...
	public function getListQuery()
	{
		...
		JPUserHelper::filterByViewAccess($query,'project','time');
	...
	/*
	 * Filter by predefined date type or date range
	 */
	private function dateRangeFilter(&$query){

		$filter_start_date      = $this->getState('filter.start_date', null);
		$filter_end_date        = $this->getState('filter.end_date', null);
		$filter_date_range_type = $this->getState('filter.date_range_type', null);

		if ($filter_date_range_type > 0 && $filter_date_range_type != 7) // using predefined range date
		{
			$rangeDates = $this->geRangeDates($filter_date_range_type);

			$query->where("a.log_date between '{$rangeDates['first']} 00:00:00' AND '{$rangeDates['last']} 23:59:59'");

			$filter_start_date      = $this->setState('filter.start_date', null);
			$filter_end_date    = $this->setState('filter.end_date', null);
	...
	public function getItems()
	{
		...
		$global_params = ComponentHelper::getParams('com_jptime', true);
	...
	public function getAuthors()
	{
		...
		$query->join('INNER', '#__jp_timesheet AS a ON a.created_by = u.id');

		// Implement View Level Access
		if (!$user->authorise('core.admin', 'com_jptime'))
		{
	...
	public function getTasks()
	{
		...
		$query->join('INNER', '#__jp_timesheet AS a ON a.task_id = t.id');
	...
	/*
	 * Method to get first and last date based on date range type
	 *
	 * @param   int     $rangeType   1 -> this week | 2 -> this month ...
	 * @return  array   $dates
	 */
	public function geRangeDates($rangeType)
	{
		$types = [
			1 => ["monday this week", "sunday this week"],
			2 => ["first day of this month", "last day of this month"],
			3 => ['first day of january this year', 'last day of december this year'],
			4 => ["monday previous week", "sunday previous week"],
			5 => ["first day of previous month", "last day of previous month"],
			6 => ['first day of january previous year', 'last day of december previous year'],
		];
		...
		foreach ($types as $type => $value)
		{
			...
			if ($type == $rangeType)
			{
				...
				$dates['first'] = (new DateTime($value[0]))->format('Y-m-d');
				$dates['last']  = (new DateTime($value[1]))->format('Y-m-d');
