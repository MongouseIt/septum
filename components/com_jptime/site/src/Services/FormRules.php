...
namespace Joomla\Component\JPtime\Site\Service;
...
class FormRules implements RulesInterface
{
    ...
    {
        $segments = self::JPtimeBuildRoute($query);
    ...
    {
        $vars = self::JPtimeParseRoute($segments);
    ...
    /**
     * Build the route for the com_jptime component
     *
     * @param     array    $query    An array of URL arguments
     *
     * @return    array              The URL arguments to use to assemble the subsequent URL.
     */
   public static function JPtimeBuildRoute(&$query)
    {
        ...
        if($view == 'timesheet') {
            ...
            if (isset($query['filter_project'])) {
                if (strpos($query['filter_project'], ':') === false) {
                    $query['filter_project'] = self::JPtimeMakeSlug($query['filter_project'], '#__jp_projects');
                }
            }
            else {
                $query['filter_project'] = self::JPtimeMakeSlug('0', '#__jp_projects');
            }
     ...
     */
    public static function JPtimeParseRoute($segments)
    {
        // Setup vars
        $vars  = array();
        ...
        if ($vars['view'] == 'timesheet') {
            if ($count == 1) {
                $vars['filter_project'] = self::JPtimeParseSlug($segments[0]);
            }
     ...
     */
    public static function JPtimeParseSlug($segment)
    {
     ...
     */
    public static function JPtimeMakeSlug($id, $table, $alt = 'all', $field = 'alias')
    {
        if ($id == '' || $id == '0') {
