...
class JPtimeModelTimesheet extends ListModel
{
    ...
    public function getAuthors()
    {
        ...
        $query->select('u.id AS value, u.name AS text')
              ->from('#__users AS u')
              ->join('INNER', '#__jp_timesheet AS a ON a.created_by = u.id')
    ...
    public function getTasks()
    {
        ...
        $query->select('t.id AS value, t.title AS text')
              ->from('#__jp_tasks AS t')
              ->join('INNER', '#__jp_timesheet AS a ON a.task_id = t.id')
              ->where('a.project_id = ' . $this->_db->quote($project))
              ->group('t.id')
              ->order('t.title');
    ...
    protected function getListQuery()
    {
        ...
        $query->select('t.id AS task_exists')
              ->join('LEFT', '#__jp_tasks AS t ON t.id = a.task_id');
