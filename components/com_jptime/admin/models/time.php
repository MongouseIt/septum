 ...
 *
 */
class JPtimeModelTime extends AdminModel
{
    /**
     ...
     */
    public function getTable($type = 'Time', $prefix = 'JPtable', $config = array())
    {
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_jptime.time', 'time', array('control' => 'jform', 'load_data' => $loadData));
        ...
        if (!$user->authorise('core.admin', 'com_jptime') && !$user->authorise('core.manage', 'com_jptime')) {
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_jptime.edit.' . $this->getName() . '.data', array());
    ...
    protected function canDelete($record)
    {
        ...
        return $user->authorise('core.delete', 'com_jptime.time.' . (int) $record->id);
    }
    ...
    protected function canEditState($record)
    {
        ...
        return $user->authorise('core.edit.state', 'com_jptime.time.' . (int) $record->id);
    }
    ...
    {
        if (empty($record->id)) {
            return $user->authorise('core.edit', 'com_jptime');
        }

        $user  = Factory::getApplication()->getIdentity();
        $asset = 'com_jptime.time.' . (int) $record->id;
