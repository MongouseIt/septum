...
class JoomprojectSmartTags
{
    ...
    static public function getInstance($opts = [], $factory = null)
    {
		...
		if ($instance === null)
		{
            $instance = new JoomprojectSmartTags($opts, $factory);
		}
	 ...
	 * 
	 * The GROUP part is required and must be pointing to \\SmartTags\GROUP file which must declare a class with the name GROUP.
	...
	public function modifierShortNumber($modifierValue, &$subject)
	{
		if ($modifierValue)
		{
			$subject = JoomprojectGalleryManager::toShortFormat($subject);
		}
	}
