...
class JoomprojectImage
{
	...
	public static function resize($source, $width, $height, $quality = 70, $mode = 'crop', $destination = '', $unique_filename = false, $fix_orientation = true, $gif_mode = 'copy')
	{
		...
		try {
			...
			$mime = JoomprojectMimes::detectFileType($source);
			...
			if ($unique_filename)
			{
				...
				JoomprojectFile::uniquefy($destination);
			}
			...
			if ($mime === 'image/gif')
			{
				if ($gif_mode === 'copy')
				{
					JoomprojectFile::copy($source, $destination, true);
				}
				else
				{
	...
	public static function resizeByHeight($src, $height, $destination = null, $quality = 70, $unique_filename = false, $fix_orientation = true, $gif_mode = 'copy')
	{
		...
		// Determine the MIME of the original file to get the proper type
		$mime = JoomprojectMimes::detectFileType($src);
	...
	public static function resizeByWidthOrHeight($source, $width, $height, $quality = 80, $destination = '', $resize_method = 'crop', $unique_filename = false, $fix_orientation = true)
	{
		...
		{
			$resized_image = JoomprojectImage::resizeByHeight($source, $height, $destination, 80, $unique_filename, $fix_orientation);
		}
		else
		{
			/**
			...
			$resized_image = $height && !is_null($height)
				?
				JoomprojectImage::resize($source, $width, $height, 80, $resize_method, $destination, $unique_filename, $fix_orientation)
				:
				JoomprojectImage::resizeAndKeepAspectRatio($source, $width, 80, $destination, $unique_filename, $fix_orientation);
	...
	{
		$font = implode(DIRECTORY_SEPARATOR, [JPATH_SITE, 'media', 'com_joomproject', 'font', 'arial.ttf']);
