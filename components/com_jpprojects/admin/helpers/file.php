...
class JoomprojectFile
{
	...
	public static function upload($file, $upload_folder = null, $allowed_file_types = [], $allow_unsafe = false, $random_prefix = null, $random_suffix = false)
	{
		...
		{
			self::error(Text::sprintf('COM_JOOMPROJECT_GALLERY_UPLOAD_ERROR_CANNOT_UPLOAD_FILE', $file['name']));
		}
		...
		{
			$file_data = JoomprojectFile::pathinfo($filename);
	...
	public static function move($source_file, $destination_file, $replace_existing = false, $hash = false)
	{
		...
		{
			self::error(Text::sprintf('COM_JOOMPROJECT_GALLERY_CANNOT_CREATE_FOLDER', $destination_folder));
		}
		...
		{
			self::error(Text::sprintf('COM_JOOMPROJECT_GALLERY_CANNOT_MOVE_FILE', $destination_file));
		}
	...
	public static function checkMimeOrDie($allowed_types, $file_object)
	{
		...
		if (!JoomprojectMimes::validateFileExtension($fileExtension, $allowed_types))
		{
			self::error(Text::sprintf('COM_JOOMPROJECT_GALLERY_UPLOAD_INVALID_FILE_EXT', $safeFilename, $fileExtension, $allowed_types));
		}

		// Do we have a mime type detected?
		if (!$mime_type = JoomprojectMimes::detectFileType($file_path))
		{
			self::error(Text::sprintf('COM_JOOMPROJECT_GALLERY_UPLOAD_NO_MIME_TYPE', $safeFilename));
		}

		if (!JoomprojectMimes::check($allowed_types, $mime_type))
		{
			self::error(Text::sprintf('COM_JOOMPROJECT_GALLERY_UPLOAD_INVALID_FILE_TYPE', $safeFilename, $mime_type, $allowed_types));
		}
	}
