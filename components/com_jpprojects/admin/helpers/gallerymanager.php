...
class JoomprojectGalleryManager
{
	/**
	...
	public static function upload($file, $upload_settings, $media_uploader_file_data, $resizeSettings)
	{
		...
		try {
			$source = JoomprojectFile::upload($file, self::getFullTempFolder(), $upload_settings['allowed_types'], $upload_settings['allow_unsafe']);
		...
		if ($resizeSettings['original_image_resize'])
		{
			...
			{
				$original_image_full = JoomprojectImage::resize($source, $resizeSettings['original_image_resize_width'], $resizeSettings['original_image_resize_height'], 70, 'crop', $original_image_destination, true);
			}
			else if ($resizeSettings['original_image_resize_width'])
			{
				$original_image_full = JoomprojectImage::resizeAndKeepAspectRatio($source, $resizeSettings['original_image_resize_width'], 70, $original_image_destination, true);
			}
			else if ($resizeSettings['original_image_resize_height'])
			{
				$original_image_full = JoomprojectImage::resizeByHeight($source, $resizeSettings['original_image_resize_height'], $original_image_destination, 70, true);
			}
		...
		else
		{
			...
			$original_image_full = JoomprojectFile::move($source, $original_image_destination);
			$original_image_relative = str_replace(JPATH_ROOT . $ds, '', $original_image_full);
		}
		...
		if (isset($upload_settings['watermark']['type']) && $upload_settings['watermark']['type'] !== 'disabled')
		{
			...
			$source_image_full = JoomprojectFile::copy($original_image_full, $source_file_path, false, true);
	 ...
	 * @return  void
	 */
	public static function moveTempItemsToDestination($value, $fieldparams, $destination_folder)
	{
		...
		if (!JoomprojectFile::createDirs($destination_folder))
		{
			return;
		}
		...
		{
			$ffieldparams = new Registry($fieldparams);
		}
		...
		$limit_files = (int) $fieldparams->get('limit_files', 0);
		...
		foreach ($items as $key => &$item)
		{
			...
			if (JoomprojectFunctions::startsWith($item['thumbnail'], self::getTempFolder()) && file_exists($thumbnail_path))
			{
				// Move thumbnail
				$thumb = JoomprojectFile::move($thumbnail_path, $destination_folder . $thumbnail_clean);
			...
			// Move original image
			if (JoomprojectFunctions::startsWith($item['image'], self::getTempFolder()) && file_exists($image_path))
			{
				// Move image
				$image = JoomprojectFile::move($image_path, $destination_folder . $image_clean);
			...
			if (!$item['source'] && $fieldparams->get('watermark.type', 'disabled') !== 'disabled')
			{
				// Create source from original image
				$source = JoomprojectFile::copy($image_path, $image_path, false, true);
			...
			if (JoomprojectFunctions::startsWith($item['source'], self::getTempFolder()) && file_exists($source_path))
			{
				// Move source
				$thumb = JoomprojectFile::move($source_path, $destination_folder . $source_clean);
	...
	private static function updateDestinationPath(&$item, $destination_folder)
	{
		...
		$item = array_merge($item, [
			'source' => !empty($item['source']) && !JoomprojectFunctions::startsWith($item['source'], $destination_folder) ? implode($ds, [$destination_folder, $item['source']]) : $item['source'],
			'thumbnail' => !JoomprojectFunctions::startsWith($item['thumbnail'], $destination_folder) ? implode($ds, [$destination_folder, $item['thumbnail']]) : $item['thumbnail'],
			'image' => !JoomprojectFunctions::startsWith($item['image'], $destination_folder) ? implode($ds, [$destination_folder, $item['image']]) : $item['image']
		]);
	...
	public static function generateThumbnail($source = '', $destination = '', $resizeSettings = [], $destination_folder = null, $unique_filename = true)
	{
		...
		{
			$resized_image = JoomprojectImage::resizeByHeight($source, $thumb_height, $destination, 70, $unique_filename, true, 'resize');
		}
		else
		{
			...
			$resized_image = $thumb_height && !is_null($thumb_height)
				?
				JoomprojectImage::resize($source, $thumb_width, $thumb_height, 70, $resizeSettings['thumb_resize_method'], $destination, $unique_filename, true, 'resize')
				:
				JoomprojectImage::resizeAndKeepAspectRatio($source, $thumb_width, 70, $destination, $unique_filename, true, 'resize');
	 ...
	 * @return object
	 */
	public static function getModuleData($value, $selector = 'id', $options = ['access' => 1])
	{
		if (!$value)
		{
			return;
