...
JLoader::register('JPtableProjectCompat', dirname(__FILE__) . '/project_compat.php');


/**
 * Joomproject Project Table
 *
 */
class JPtableProject extends JPtableProjectCompat
{
    ...
    {
        parent::__construct('#__jp_projects', 'id', $db);
    ...
    {
        $k = $this->_tbl_key;
        return 'com_jpprojects.project.' . (int) $this->$k;
    }
    ...
    protected function _getAssetParentIdCompat($table = null, $id = null)
    {
        ...
        if ($this->catid) {
            $query->select('id')
                  ->from('#__assets')
                  ->where('name = ' . $this->_db->quote("com_jpprojects.category." . (int) $this->catid));
        ...
        if (!$asset_id) {
            $query->select('id')
                  ->from('#__assets')
                  ->where('name = ' . $this->_db->quote("com_jpprojects"));
    ...
    public function check()
    {
       ...
       if (empty($this->title)) {
		...
		if (empty($this->title)) $this->alias = $this->title;
        ...
        if (!MailHelper::isEmailAddress($email)) $registry->set('email', '');
    ...
    public function store($updateNulls = false)
    {
        ...
        $this->start_date = empty($this->start_date) ? null : $this->start_date;
        $this->end_date = empty($this->end_date) ? null : $this->end_date;

        // Verify that the alias is unique
        $table = Table::getInstance('Project', 'JPtable');

        if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0)) {
            $this->setError(Text::_('COM_JOOMPROJECT_ERROR_PROJECT_UNIQUE_ALIAS'));
            return false;
        }
    ...
    /**
     * Method to store the project asset for the other JP components
     *
    ...
    public function storeComponentAssets($pk = null)
    {
        ...
        $query      = $this->_db->getQuery(true);
        $ignore     = array('com_joomproject', 'com_jpprojects');
        ...
        {
            if (!JPApplicationHelper::usesProjectAsset($name)) continue;
            ...
            if ($project_asset) {
                ...
                if (isset($this->asset_rules[$name])) {
                    ...
                    if ($this->asset_rules[$name] instanceof Rules) {
	...
	public function setComponentRules($component, $input)
	{
		...
		if ($input instanceof JAccessRules) {
			...
			$this->asset_rules[$component] = new Rules($input);
		}
	}
}
