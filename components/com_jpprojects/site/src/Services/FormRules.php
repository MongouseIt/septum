...
namespace Joomla\Component\JPprojects\Site\Service;
...
class FormRules implements RulesInterface
{
    ...
    {
        $segments = self::JPprojectsBuildRoute($query);
    ...
    {
        $vars = self::JPprojectsParseRoute($segments);
    ...
    /**
     * Build the route for the com_jpprojects component
     *
     * @param     array    $query    An array of URL arguments
     *
     * @return    array              The URL arguments to use to assemble the subsequent URL.
     */
    public static function JPprojectsBuildRoute(&$query)
    {
        ...
        if($view == 'projects') {
            ...
            if (isset($query['filter_category'])) {
                if (strrpos($query['filter_category'], ':') === false) {
                    $query['filter_category'] = self::JPprojectsMakeSlug($query['filter_category'], '#__categories');
                }
        ...
        if($view == 'form') {
            if (isset($query['id'])) {
                if (strrpos($query['id'], ':') === false) {
                    $query['id'] = self::JPprojectsMakeSlug($query['id'], '#__jp_projects');
                }
     ...
     */
    public static function JPprojectsParseRoute($segments)
    {
        ...
        if ($vars['view'] == 'projects') {
            if ($count >= 2) {
                $vars['filter_category'] = self::JPprojectsParseSlug($segments[1]);
            }
     ...
     */
    public static function JPprojectsParseSlug($segment)
    {
     ...
     */
    public static function JPprojectsMakeSlug($id, $table, $alt = 'all', $field = 'alias')
    {
        if ($id == '' || $id == '0') {
            return '';
        }
