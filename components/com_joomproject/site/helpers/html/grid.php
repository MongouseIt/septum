...
abstract class JHtmlGrid
{
	...
	public static function sort($title, $order, $direction = 'asc', $selected = '', $task = null, $new_direction = 'asc', $tip = '', $form = null)
	{
		...
		$html = '<a href="#" onclick="Joomla.tableOrdering(\'' . $order . '\',\'' . $direction . '\',\'' . $task . '\'' . $form . ');return false;"'
			. '  data-bs-toggle="popover" title="' . htmlspecialchars(Text::_($tip ?: $title)) . '"'
		...
		if ($order == $selected)
		{
			$html .= ' <span class="text-muted fas fa-' . $icon[$index] . '"></span>';
		}
	...
	public static function checkall($name = 'checkall-toggle', $tip = 'JGLOBAL_CHECK_ALL', $action = 'Joomla.checkAll(this)')
	{
		...
		return '<input type="checkbox" name="' . $name . '" value="" data-bs-toggle="popover" title="' . HTMLHelper::_('tooltipText', $tip)
			. '" onclick="' . $action . '" />';
	}
	...
	public static function order($rows, $image = 'filesave.png', $task = 'saveorder')
	{
		return '<a href="javascript:saveorder('
			. (count($rows) - 1) . ', \'' . $task . '\')"  data-bs-toggle="tooltip" class="saveorder btn btn-micro float-end" title="'
	...
	protected static function _checkedOut(&$row, $overlib = true)
	{
		...
		if ($overlib)
		{
			...
			$hover = '<span class="editlinktip"  data-bs-toggle="popover" title="' . HTMLHelper::_('tooltipText', 'JLIB_HTML_CHECKED_OUT', $row->editor)
				. '<br />' . $date . '<br />' . $time . '">';
		}
