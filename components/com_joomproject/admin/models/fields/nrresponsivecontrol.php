...
class JFormFieldNRResponsiveControl extends TextField
{
	...
	protected $type = 'NRResponsiveControl';
	...
	private function getSubtypeFieldsData()
	{
		...
		$layout = new FileLayout('responsive_control_item');
...
;
			...
			else
			{
				if ($device === 'desktop')
				{
					...
					if ($is_dimension_control)
					{
						...
						$value = Control::parseInputValue($input_value, $input_type);
						$value['linked'] = '1';
					}
					else
					{
						$value = Control::findUnitInValue($input_value);
					}
				}
			}
		}
	...
	protected function getSubtypeHTML($name, $type, $device, $value)
	{
		...
		if ($subtype_options)
		{
			...
			if (is_array($subtype_options))
			{
				...
				if ($device === 'desktop' && ($inherit_item_key = array_search('COM_JOOMPROJECT_GALLERY_INHERIT', $subtype_options)) !== false)
				{
	...
	{
		HTMLHelper::stylesheet('com_joomproject/responsive_control.css', ['relative' => true, 'version' => 'auto']);
		HTMLHelper::script('com_joomproject/responsive_control.js', ['relative' => true, 'version' => 'auto']);
		...
		// Render layout
		$layout = new FileLayout('responsive_control',JPATH_ADMINISTRATOR . '/components/com_jpprojects/layouts/' );
	...
	private function getSubformFieldsData()
	{
		...
		$layout = new FileLayout('responsive_control_item', JPATH_ADMINISTRATOR . '/components/com_jpprojects/layouts/');
