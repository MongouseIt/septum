...
HTMLHelper::stylesheet('com_joomproject/unit_selector.css', ['relative' => true, 'version' => 'auto']);

$multipleUnits = count($units) > 1;

if ($multipleUnits)
{
	HTMLHelper::script('com_joomproject/unit_selector.js', ['relative' => true, 'version' => 'auto']);
}
?>
<div class="tf-unit-control-dropdown<?php echo $multipleUnits ? ' has-multiple-units' : ''; ?><?php echo isset($dropdown_class) && !empty($dropdown_class) ? ' ' . $dropdown_class : ''; ?>">
	<div class="tf-unit-control-dropdown--opener"<?php echo count($units) > 1 ? ' title="' . Text::_('COM_JOOMPROJECT_GALLERY_SELECT_UNIT') . '"' : ''; ?>>
	...
	if (!isset($hide_input))
	{
		$layout = new FileLayout('input_value', JPATH_ADMINISTRATOR . '/components/com_jpprojects/layouts/controls/unit');
