...
class JPremindersModelReminders extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        $filter_project = $this->getState('filter.project_id');
        ...
        $query->select(
            $this->getState(
                'list.select',
                'r.id,r.task_id,t.title,t.alias,r.types,r.assigned_users,u.name,u.username,r.created_by,r.description,r.start_date, r.amount,r.state,r.repeats,t.project_id,r.checked_out,r.checked_out_time')
        );
        ...
        if (isset($filter_task) && !is_null($filter_task)) {
        ...
        if (is_numeric($filter_project) && $filter_project > 0) {
            $query->where('t.project_id = ' . (int) $filter_project);
        }
        // Filter by author
        ...
        elseif (is_array($filter_author))
        {
            $filter_author = ArrayHelper::toInteger($filter_author);
            $filter_author = implode(',', $filter_author);
            $query->where('r.created_by IN (' . $filter_author . ')');
        }
