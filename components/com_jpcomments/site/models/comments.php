 ...
 *
 */
class JPcommentsModelComments extends ListModel
{
    /**
     * Constructor.
     *
    ...
    public function getListQuery()
    {
        ...
        $query->from('#__jp_comments AS a');

        // Add the level in the tree.
        /*$query->select('COUNT(DISTINCT c2.id) AS level');
        $query->join('LEFT OUTER', $db->quoteName('#__jp_comments') . ' AS c2 ON '
        ...
        $query->join('LEFT', $db->quoteName('#__jp_comments') . 'AS r on r.parent_id = a.id');*/
	    ...
	    JPUserHelper::filterByViewAccess($query,'project','comments');
    ...
    public function getItems()
    {
        ...
        $global_params = ComponentHelper::getParams('com_jpcomments', true);
        ...
        foreach ($items as $i => &$item)
        {
            ...
            // Create slug
            // $items[$i]->slug = $items[$i]->title ? ($items[$i]->id . ':' . Joomla\CMS\Application\ApplicationHelper::stringURLSafe($items[$i]->title)) : $items[$i]->id;
    ...
    protected function populateState($ordering = 'a.lft', $direction = 'ASC')
    {
        ...
        $params = ComponentHelper::getParams('com_jpcomments');
        ...
        // Filter - Project
        $value = JPApplicationHelper::getActiveProjectId();
