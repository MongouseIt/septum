        ...
        var core = {
            ...
            render: function (element) {
                ...
                if (settings.scrollToToday) {
                    ...
                    /*$('.nav-slider-content').trigger('mousedown');
                ...
                settings.onRender(element, core);
            },
            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        entries.push('<span class="fn-label gantt-bs-tt' + (entry.cssClass ? ' ' + entry.cssClass : '') + '" title="' + $(entry.name).text() + '">' + entry.name + '</span>');
                        entries.push('</div>');
            ...
            // the entire chart
            createProgressBar: function (days, cls, desc, label, dataObj, id, element) {
                var cellWidth = tools.getCellSize();
                var barMarg = tools.getProgressBarMargin() || 0;
                var t = $('<span>' + label + '</span>').text();
                var bar = $('<div id="'+ id +'" class="bar" ' + (t ? ' title="' + t + '"' : '') + '><div class="fn-label">' + label + '</div></div>')
                        .addClass(cls)
                        .css({
            ...
            createDependency: function(positionStart, positionEnd, idStart, idEnd){
              ...
              if (Math.abs(hDist) < 8 ){
                $.extend(css,{'margin-left': positionStart.left -4, width: 8, 'margin-top': positionStart.top})
              }
              else {
                // down
               if ( vDist > 0){
                  $.extend(css,{'margin-top': positionStart.top})
                }
                // Up
                else{
                  $.extend(css,{'margin-top': positionEnd.top })
                }
                // left
               if (hDist < 0){
                  $.extend(css,{'margin-left': positionStart.left - Math.abs(hDist) - 5, width: Math.abs(hDist) + 9})
                }
                // Right
                else {
                  $.extend(css,{'margin-left': positionStart.left , width: Math.abs(hDist)})
                }
              }
              return $dep.css(css)
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                case "hours":
                                    ...
                                    if (day.dep && day.id && settings.depHover) {
                                        _bar.attr('data-dependencies', day.dep.join(','));

                                        _bar.mouseover(function (e) {
                                              var deps = $(this).attr('data-dependencies').split(',');
                                              var did  = this.id;
                                              var c    = deps.length;

                                              for(var i = 0; i < c; i++)
                                              {
                                                   var ddiv = $('#' + deps[i] + '-' + did + 'Dep');

                                                   if (ddiv.length) ddiv.css('display', 'block');
                                              }
                                          ...
                                          .mouseout(function () {
                                              ...
                                              for(var i = 0; i < c; i++)
                                              {
                                                   ...
                                                   if (ddiv.length) ddiv.css('display', 'none');
                                              }
                                          });
                                    }
                      ...
                      if (day.id && day.dep){
                        $.each($.map([day.dep], function(dep){ return dep }), function (j, dep) {
                          ...
                          if ($e1.length && $e2.length){
                            ...
                            var p1 = {left: $e1.outerWidth() + tools.pxToInt($e1.css('margin-left')), top: tools.pxToInt($e1.css('margin-top')) + $e1.outerHeight()/2 },
                                p2 = {left: tools.pxToInt($e2.css('margin-left')), top: tools.pxToInt($e2.css('margin-top'))+ $e2.outerHeight()/2 };
            ...
            wheelScroll: function (element, e) {
                ...
                if (element.canWheel) {
                    if (delta > 0) {
                        // Page down
                        if (element.pageNum + 1 <= element.pageCount) {
                            element.canWheel = false;
                            core.navigatePage(element, 1);
                        }
                    }
                    else {
                        // Page up
                        if (element.pageNum - 1 <= 0) {
                            element.canWheel = false;
                            core.navigatePage(element, -1);
                        }
                    }
            ...
            dragDataPanelMove: function (element, screenX, e)
            {
                if (typeof element.scrollNavigation.dragStartX !== 'undefined') {
                   // Update dragging of panel no more than 30 times a second, to smooth out
                   // choppy dragging.
        ...
        var tools = {
            ...
            genId: function (ticks) {
                ...
                switch (settings.scale) {
                    ...
                    default:
                        return (new Date(t.getFullYear(), t.getMonth(), t.getDate(), 0, 0)).getTime();
                }
            },
