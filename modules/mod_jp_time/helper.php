 ...
 *
 */
abstract class modJPtimeHelper
{

	public static function getAjax(){

		$action = Factory::getApplication()->input->get('action',null,'word');
	...
	public static function loadMedia()
    {
        ...
        if (Factory::getDocument()->getType() == 'html') {
            HTMLHelper::_('script', 'mod_jp_time/jquery.fileDownload.js', array('version' => 'auto', 'relative' => true));
        }
    }
	public static function exportCSV()
	{
		$dispatcher	= new  Joomla\CMS\Application\SiteApplication();
        $dispatcher->registerEvent('onBeforeCompileHead', 'prepareData');
	}

	public static function displayData()
	{
		$dispatcher	= new  Joomla\CMS\Application\SiteApplication();
        $dispatcher->registerEvent('onBeforeRender', 'prepareData');
        prepareData();
	}
}

class moduleHelperPagination extends Pagination
{
...
function displayData($items)
{
	...
	$module 			= ModuleHelper::getModule('mod_jp_time');
	$params 			= new Registry();
	...
	$app 				= Factory::getApplication();
	$limitstart 		= (int) \Joomla\CMS\Factory::getApplication()->input->get('limitstart',0);
	$stotal_time		= 0;
	$total_time			= 0;

	$html ='
		<div class="cat-list-row">
		<ul class="list-tasks list-group my-4" id="tasklist">';
    ...
    foreach ($items AS $i => $item) {
		...
		$html .= '
			<li id="list-item-'.$x.'" class="list-group-item">
				<div class="task-row d-flex justify-content-between  align-items-center">
					<span class="task-title">';
						if ($exists) {
							$html .= '<a  class="" href="'.Route::_(JPtasksHelperRoute::getTaskRoute($item->task_slug, $item->project_slug, $item->milestone_slug, $item->list_slug)).'"
						  data-bs-toggle="popover"  data-trigger="hover"  title="'. htmlspecialchars($item->task_title, ENT_QUOTES, "UTF-8").'" data-content="'. htmlspecialchars($item->description, ENT_QUOTES, "UTF-8").'">'
						 . htmlspecialchars($item->task_title, ENT_QUOTES, 'UTF-8'). '</a>';
							...
							$menu->end();
							$html .= $menu->render(array('class' => 'btn-sm btn-link'));
					...
					$html .= '</span>
					<span class="text-muted">' . HTMLHelper::_('time.format', $item->log_time). '</span>
				</div>
				<div id="collapse-'.$x.'" class="collapse">
					<hr />
					<small class="task-description">' . Text::_('COM_JOOMPROJECT_FIELD_PROJECT_LABEL').':'.htmlspecialchars($item->project_title, ENT_COMPAT, 'UTF-8').'</small>';

					if ($show_author) {
						$html .= '<span class="label user">
							<span aria-hidden="true" class="fas fa-user text-white"></span>' . htmlspecialchars($item->author_name, ENT_COMPAT, 'UTF-8') . '</span>
						<input type="hidden" id="assigned'. $i.'" name="assigned['.$item->id.']" />';
					}

					if ($show_date) {
						$html .= '<span class="badge bg-success"><span aria-hidden="true" class="icon-calendar"></span>' . HTMLHelper::_('date', $item->log_date, Text::_('DATE_FORMAT_LC4')) . '</span>';
					}

					if ($show_monetary) {
						$html .= '<span class="badge bg-dark">'.Text::_('COM_JOOMPROJECT_TIME_TRACKING_RATE').': '.HTMLHelper::_('jphtml.format.money', $item->rate).'</span>';
						if (!$item->billable) {
							$html .= '<span class="badge bg-info">'.Text::_('COM_JOOMPROJECT_TIME_TRACKING_UNBILLABLE').': '.HTMLHelper::_('jphtml.format.money', $item->billable_total).'</span>';
						}
						else {
							$html .= '<span class="badge bg-success">'.Text::_('COM_JOOMPROJECT_TIME_TRACKING_BILLABLE').': '.HTMLHelper::_('jphtml.format.money', $item->billable_total).'</span>';
						}
					}
	...
	$html .= '<li id="list-item-'.$x++.'" class="list-group-item">';
	$html .= '<div class="task-row clearfix">';
	if ($total_time == $stotal_time) {
		$html .= '			<span class="task-title">'.Text::_('MOD_JP_TIME_TOTAL').'</span><span class="float-end">'.HTMLHelper::_('time.format', $total_time).'</span>';
	}
	else {
		$html .= '			<span class="task-title">'.Text::_('MOD_JP_TIME_TOTAL').'</span><span class="float-end">'.HTMLHelper::_('time.format', $stotal_time).' / '.HTMLHelper::_('time.format', $total_time).'</span>';
	}
	$html .= '</div></li>';
	$html .= '	</ul>
	</div>';

	$pagination = new moduleHelperPagination( count($items), $limitstart, $list_limit );
	$html .= LayoutHelper::render(
		'common.modPagination',
		['pagination' => $pagination],
		'',
		['client' => 'site','component' => 'com_joomproject']
	);
...
function prepareData()
	...
	$action 			= \Joomla\CMS\Factory::getApplication()->input->get('action',null);

	$filter_author 		= $params->get('filter_own',null);
	$filter_start_date 	= \Joomla\CMS\Factory::getApplication()->input->get('filter_start_date',null);
	$filter_end_date 	= \Joomla\CMS\Factory::getApplication()->input->get('filter_end_date',null);
	$filter_project 	= \Joomla\CMS\Factory::getApplication()->input->get('filter_project',null);
	...
	$query->from('#__jp_timesheet AS a');
	    ...
	    JPUserHelper::filterByViewAccess($query,'project','time','time');
	...
	if ($filter_start_date && $filter_end_date) {
		$query->where("a.log_date between '$filter_start_date 00:00:00' AND '$filter_end_date 23:59:59'");
	}
	else if ($filter_start_date) {
		$query->where("a.log_date >= '$filter_start_date 00:00:00'");
	}
	else if ($filter_end_date) {
		$query->where("a.log_date <= '$filter_end_date 23:59:59'");
	}
...
function exportData($taskdata)
{
	...
	$csv_header = array(
		Text::_('MOD_JP_TIME_CSV_PROJECT'),
		Text::_('MOD_JP_TIME_CSV_TITLE'),
		Text::_('MOD_JP_TIME_CSV_DESC'),
		Text::_('MOD_JP_TIME_CSV_MILE'),
		Text::_('MOD_JP_TIME_CSV_TLKIST'),
		Text::_('MOD_JP_TIME_CSV_AUTHOR'),
		Text::_('MOD_JP_TIME_CSV_DATE'),
		Text::_('MOD_JP_TIME_CSV_RATE'),
		Text::_('MOD_JP_TIME_CSV_HRS'),
		Text::_('MOD_JP_TIME_CSV_TOTAL'),
		Text::_('MOD_JP_TIME_CSV_BILLABLE')
	);
	...
	foreach ($taskdata as $result){
		...
		if ($user->authorise('core.edit','com_jptime') || $show_author) {
		...
		if ($user->authorise('core.edit','com_jptime') || $show_date) {
			$csv 		.= "$logdate,";
		}
		else {
			$csv 		.= ",";
		...
		if ($user->authorise('core.edit','com_jptime') || $show_monetary) {
			$csv 		.= "$result->rate,";
		}
		else {
			$csv		.= ",";
