...
class plgJPactivitiesHelper
{
    ...
    public function translateItem($item)
    {
        ...
        // translate name
        $this->item->nameTranslated = Text::_(strtoupper($item->extension) . '_UA_' . strtoupper($item->name));
    ...
    public function translateGroup($group)
    {
        ...
        if (!isset($this->cache_token[$tkey])) {
            $this->cache_token[$tkey] = Text::_($tkey);
        }
    ...
    protected function getUserLink()
    {
        if ($this->user_link == 'joomla' || empty($this->user_link)) {
            ...
            $link = JPactivitiesHelperUserLink::get($this->item->created_by, $this->item->username, $this->user_link);
        }
