 ...
 * @subpackage   plg_content_jptasks
 *
 * @author       JoomBoost
...
/**
 * Joomproject Tasks Content Plugin Class
 *
 */
class plgContentJPtasks extends CMSPlugin
{
    /**
    ...
    protected $contexts = array(
        ...
        'com_jpmilestones.milestone', 'com_jpmilestones.form',
        'com_jptasks.tasklist', 'com_jptasks.tasklistform'
    );
     ...
     * @return    boolean                True
     */
    public function onContentBeforeSave($context, $table, $is_new ,$data)
    {
        // Do nothing if the plugin is disabled
        if (!PluginHelper::isEnabled('content', 'jptasks')) return true;
        ...
        if (in_array($context, array('com_jpmilestones.milestone', 'com_jpmilestones.form'))) {
            $db    = Factory::getDbo();
    ...
    public function onContentAfterSave($context, $table, $is_new = false)
    {
        ...
        if (in_array($context, array('com_jpprojects.project', 'com_jpmilestones.milestone', 'com_jptasks.tasklist'))) {
            // Update access
        ...
        if ($context == 'com_jpmilestones.milestone') {

            $start_changed = ($table->start_date != $this->prev_start);
            $end_changed   = ($table->end_date != $this->prev_end);
            $end_changed_request = ($table->end_date_request != $this->prev_end);
            // Update start and end dates on tasks and project
            if ($start_changed || $end_changed || $end_changed_request) {
                $this->updateTimeline($table->id, $table->start_date, $table->end_date,$table->end_date_request);
            }
        }
    ...
    protected function deleteFromContext($context, $id)
    {
        ...
        if (!$imported) {
            ...
            JLoader::register('JPtableTask', JPATH_ADMINISTRATOR . '/components/com_jptasks/tables/task.php');
            JLoader::register('JPtableTasklist', JPATH_ADMINISTRATOR . '/components/com_jptasks/tables/tasklist.php');

            $imported = true;
        }

        $task_table = Table::getInstance('Task', 'JPtable');
        $list_table = Table::getInstance('Tasklist', 'JPtable');
        ...
        $fields = array(
            ...
            'com_jpmilestones.milestone' => 'milestone_id',
            'com_jptasks.tasklist'       => 'list_id'
        );
    ...
    protected function updateAccess($context, $id, $access)
    {
        ...
        if ($context == 'com_jptasks.tasklist') return;
     ...
     * @return    void
     */
    protected function updateTimeline($id, $start = null, $end = null,$endRequest = null)
    {
        jimport('joomproject.library');

        // Get component parameters
        $params = ComponentHelper::getParams('com_jpmilestones');
        $autoExtendTasks = $params->get('auto_extend_tasks', 0); // 0 = disabled, 1 = enabled
        $autoExtendProject = $params->get('auto_extend_project', 0); // 0 = disabled, 1 = enabled
        ...
        // Check if milestone date is postponed
        $is_postponed = $has_end && $this->prev_end && $end > $this->prev_end;

        // Update project deadline if enabled and this milestone's end date is later
        if ($autoExtendProject) {

            $has_end2    = !(empty($endRequest)   || $endRequest == $nd);
            $is_postponed2 = $has_end && $this->prev_end && $endRequest > $this->prev_end;

            // postponed milestone then update project deadline date
            if($is_postponed2){

                // set new end date
                $is_postponed = $is_postponed2;
                $has_end = $has_end2;
                $end = $endRequest;


                // Get current milestone's project ID and project's current deadline
                $query
                    ->select('m.project_id, p.end_date')
                    ->from('#__jp_milestones AS m')
                    ->join('LEFT', '#__jp_projects AS p ON p.id = m.project_id')
                    ->where('m.id = ' . (int) $id);

                $db->setQuery($query);
                $project = $db->loadObject();

                if ($project && (!$project->end_date || $project->end_date == $nd || $end > $project->end_date)) {
                    // Update project deadline
                    $query->clear()
                        ->update('#__jp_projects')
                        ->set('end_date = ' . $db->quote($end))
                        ->where('id = ' . (int) $project->project_id);
        ...
        // Get all tasks
        $query->select('id, start_date, end_date, complete')
              ->from('#__jp_tasks')
        ...
        foreach ($tasks AS $i => $item)
        {
            ...
            if ($autoExtendTasks && $is_postponed) {

                // Skip closed/completed tasks when milestone is postponed
                if ($item->complete === 1) continue;

                // When milestone is postponed, extend all open tasks to new milestone end date
                if ($item_has_end && $item->end_date != $end) {
                    $updates[] = 'end_date = ' . $db->quote($end);
                    $item->end_date = $end;
                }

                // Optionally adjust start date if needed
                if ($item_has_start && $item->start_date > $end) {
                    $updates[] = 'start_date = ' . $db->quote($end);
                    $item->start_date = $end;
                }
                ...
                // Shift dates
                list($new_start, $new_end) = JPDate::shiftTimeline($span, $this_const, $prev_const);
    ...
    protected function updateParentAsset($project)
    {
        ...
        $query->select('id')
              ->from('#__assets')
              ->where('name = ' . $db->quote('com_jptasks'));
        ...
        $query->select('id')
              ->from('#__assets')
              ->where('name = ' . $db->quote('com_jptasks.project.' . $project));
    ...
    protected function deleteProjectAsset($project)
    {
        ...
        $query->delete('#__assets')
              ->where('name = ' . $db->quote('com_jptasks.project.' . (int) $project));
