 ...
 * @subpackage   plg_content_jpprojects
 *
 * @author       JoomBoost
...
/**
 * Joomproject Projects Content Plugin Class
 *
 */
class plgContentJPprojects extends CMSPlugin
{
    /**
     ...
     * @var    array
     */
    protected $contexts = array('com_jpprojects.project', 'com_jpprojects.form');
    ...
    public function onContentBeforeSave($context, $table, $is_new = false)
    {
        ...
        if (!PluginHelper::isEnabled('content', 'jpprojects')) {
            return true;
        }
    ...
    public function onContentAfterSave($context, $table, $is_new = false)
    {
        ...
        // Do nothing if the plugin is disabled
        if (!PluginHelper::isEnabled('content', 'jpprojects')) return true;
        ...
        if ($is_new) {
            ...
            $context = ($app->isClient('administrator') ? "com_jpprojects.copy.project.id" : "com_jpprojects.copy.form.id");
            ...
            $old_table = Table::getInstance('Project', 'JPtable');
    ...
    protected function updateTimeline($project, $start = null, $end = null)
    {
        ...
        foreach ($tasks AS $i => $item)
        {
            ...
            // Shift dates
            list($new_start, $new_end) = JPDate::shiftTimeline($span, $c_constraints, $p_constraints);
    ...
    {
        // Copy milestones
        if (JPApplicationHelper::exists('com_jpmilestones')) {
    ...
    protected function copyMilestones($old_table, $new_table)
    {
        ...
        foreach ($cid AS $id)
        {
            ...
            $table->access     = ($new_table->access != $old_table->access ? $new_table->access : $table->access);
    ...
    protected function copyLists($old_table, $new_table, $milestones)
    {
        ...
        $id_map  = array();
        $table   = Table::getInstance('Tasklist', 'JPtable');
    ...
    protected function copyTasks($old_table, $new_table, $milestones, $lists)
    {
        ...
        foreach ($cid AS $id)
        {
            ...
            foreach ($old_users AS $uid)
            {
                $obj = new stdClass();
                $obj->id = null;
                $obj->item_type = 'com_jptasks.task';
                $obj->item_id   = $table->id;
                $obj->user_id   = $uid;

                $db->insertObject('#__jp_ref_users', $obj);
            }
        }
    ...
    protected function copyTaskDependencies($old_id, $new_id, $tasks)
    {
        ...
        foreach ($old_dep AS $dep)
        {
            ...
            $db->insertObject('#__jp_ref_tasks', $obj);
        }
    ...
    protected function copyLabelRefs($old_id, $new_id, $ms_map, $t_map)
    {
        ...
        foreach ($old_refs AS $old_ref)
        {
            ...
            $db->insertObject('#__jp_ref_labels', $obj);
        }
    ...
    protected function copyObservers($old_id, $new_id, $ms_map, $t_map)
    {
        ...
        foreach ($old_obs AS $old_ob)
        {
            ...
            $db->insertObject('#__jp_ref_observer', $obj);
        }
