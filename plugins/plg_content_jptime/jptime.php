 ...
 *
 */
class plgContentJPtime extends CMSPlugin
{
    /**
    ...
    public function onContentAfterSave($context, $table, $is_new = false)
    {
        ...
        if (!PluginHelper::isEnabled('content', 'jptime')) return true;
    ...
    protected function deleteFromProject($id)
    {
        ...
        if (!$imported) {
            ...
            JLoader::register('JPtableTime', JPATH_ADMINISTRATOR . '/components/com_jptime/tables/time.php');

            $imported = true;
        }

        $table = Table::getInstance('Time', 'JPtable');
        if (!$table) return;
    ...
    /**
     * Method to change the hierarchy of all time assets
     *
    ...
    protected function updateParentAsset($project)
    {
        ...
        $query->select('id')
              ->from('#__assets')
              ->where('name = ' . $db->quote('com_jptime'));
        ...
        $query->select('id')
              ->from('#__assets')
              ->where('name = ' . $db->quote('com_jptime.project.' . $project));
    ...
    protected function deleteProjectAsset($project)
    {
        ...
        $query->delete('#__assets')
              ->where('name = ' . $db->quote('com_jptime.project.' . (int) $project));
