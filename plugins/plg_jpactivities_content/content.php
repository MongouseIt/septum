 ...
 *
 */
class plgJPactivitiesContent extends CMSPlugin
{
    /**
    ...
    public function onJPactivitiesAfterSave($context, $table, $is_new)
    {
        ...
        if ($context == 'com_content.category' && $app->input->getString('extension','') != 'com_content') {
            return true;
        }
    ...
    protected function getPlugin($context)
    {
        ...
        switch ($item)
        {
            case 'article':
                ...
                $class = 'plgJPactivitiesContentArticle';
                break;

            case 'category':
                $file  = dirname(__FILE__) . '/plugins/category.php';
                $class = 'plgJPactivitiesContentCategory';
                break;
        }
