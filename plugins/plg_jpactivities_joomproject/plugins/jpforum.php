...
class plgJPactivitiesJPforum extends plgJPactivities
{
    ...
    {
        parent::onJPactivitiesAfterDelete($context, $table, $is_new, false);
    ...
    /**
     * Method to get a partial topic record
     *
     * @param     integer    $id    The topic id
     *
     * @return    object            The topic record data
     */
    ...
    /**
     * Method to get a partial reply record
     *
     * @param     integer    $id    The reply id
     *
     * @return    object            The reply record data
     */
    private function getReply($id)
    {
        static $cache = array();
        ...
        $query->select('a.asset_id, a.project_id, a.topic_id, a.state, a.access, t.title, t.alias')
              ->select('p.title AS p_title, p.alias AS p_alias')
              ->from('#__jp_replies AS a')
              ->join('left', '#__jp_topics AS t ON t.id = a.topic_id')
              ->join('left', '#__jp_projects AS p ON p.id = a.project_id')
              ->where('a.id = ' . $db->quote((int) $id));
