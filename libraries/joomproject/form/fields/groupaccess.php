...
BaseDatabaseModel::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_jpusers/models', 'JPusersModel');

/**
 * Form Field class for managing the project access rules.
 *
 */
class JFormFieldGroupAccess extends FormField
{
    /**
     * The form field type
     *
    ...
    protected function getInput()
    {
	    ...
	    $this->component = $this->element['component']   ? (string) $this->element['component']   : 'com_joomproject';
	    ...
	    if ($this->inherit) {
		    ...
		    if ($this->component == 'com_jpdesigns') {
	    ...
	    if ($this->inherit) {
		    ...
		    if (!$this->project_id) {
			    $this->project_id = (int) \Joomla\CMS\Factory::getApplication()->input->get('id');
		    }
	    }
	    ...
	    $html[] = '        <select id="aclm-add-group" autocomplete="off" class=" form-control">';
	    $html[] = '            <option value="">' . Text::_('JOPTION_SELECT_GROUP') . '</option>';
	    $html[] = implode("\n", $this->getGroupOptions());
	    $html[] = '        </select>';
	    $html[] = '        <button type="button" class="btn btn-primary" id="alcm-add-group" onclick="JPform.alcmAddSelectedGroup(\'' . $opt_url . '\');">' . Text::_('JACTION_ADD_GROUP') . '</button>';
	    $html[] = '    </div>';
    ...
    protected function getGroupsHTML()
    {
        ...
        if (Factory::getApplication()->isClient('site')) {
            require_once JPATH_SITE . '/components/com_jpusers/views/grouprules/view.raw.php';
        }
        else {
            require_once JPATH_ADMINISTRATOR . '/components/com_jpusers/views/grouprules/view.raw.php';
        }

        $db     = Factory::getDBO();
        $query  = $db->getQuery(true);
        $access = (int) $this->access;
        $model  = BaseDatabaseModel::getInstance('GroupRules', 'JPusersModel', array('ignore_request' => true));

        $config = array();
        $config['name']      = 'GroupRules';
        $config['base_path'] = JPATH_ADMINISTRATOR . '/components/com_jpusers';

        $view = new JPusersViewGroupRules($config);
        ...
        $js[] = "JPform.alcmAddPredefinedGroups(";
        $js[] = "" . $json_groups . ", ";
        $js[] = "'" . $this->component . "', ";
        $js[] = "'" . $this->section . "', ";
        $js[] = "'" . $this->inherit . "', ";
    ...
    protected function getAdminHTML($component, $section, $asset = '')
    {
        ...
        $html[] = '<ul id="rules" class="list-unstyled">';
        ...
        foreach($this->groups AS $i => $item)
        {
            ...
            // Add a checkbox
            $onclick  = 'onclick="JPform.accessGroupToggle(this);"';
            ...
            {
                if (strpos(Text::_($action->title), '-') !== false) {
                    list ($action_group, $action_name) = explode('-', Text::_($action->title), 2);
                    ...
                    $action_name = Text::_($action->title);
                }

                // Get the actual setting for the action for this group.
                $inherited = Access::checkGroup($gid, $action->name, $this->asset_id);
                $rule      = $this->rules->allow($action->name, $item->value);

                $field_id    = $this->id . '_' . $action->name . '_' . $gid;
                $field_name  = $this->name . '[' . $action->name . '][' . $gid . ']';
                $field_desc  = htmlspecialchars($action_name . '::' . Text::_($action->description), ENT_COMPAT, 'UTF-8');
                $field_title = Text::sprintf('JLIB_RULES_SELECT_ALLOW_DENY_GROUP', Text::_($action->title), trim($item->text));

                $opt_text    = Text::_(empty($item->parent_id) && empty($component) ? 'JLIB_RULES_NOT_SET' : 'JLIB_RULES_INHERITED');
                ...
                $html[] = '                             <option value="1"'.($rule === true ? ' selected="selected"' : '') . '>' . Text::_('JLIB_RULES_ALLOWED') . '</option>';
                $html[] = '                             <option value="0"' . ($rule === false ? ' selected="selected"' : '') . '>' . Text::_('JLIB_RULES_DENIED') . '</option>';
                $html[] = '                         </select>&#160; ';

                // If this asset's rule is allowed, but the inherited rule is deny, we have a conflict.
                if (($rule === true) && ($inherited === false)) $html[] = Text::_('JLIB_RULES_CONFLICT');
                ...
                if ($item->parent_id > 0) {
                    ...
                    if (Access::checkGroup($gid, 'core.admin') !== true) {
                        if ($inherited === null) {
                            $html[] = '<span class="fas fa-ban disabled"></span> ' . Text::_('JLIB_RULES_NOT_ALLOWED');
                        }
                        elseif ($inherited === true) {
                            $html[] = '<span class="fas fa-check"></span> ' . Text::_('JLIB_RULES_ALLOWED');
                        }
                        elseif ($inherited === false) {
                            if ($rule === false) {
                                $html[] = '<span class="fas fa-ban"></span> ' . Text::_('JLIB_RULES_NOT_ALLOWED');
                            }
                            else {
                                $html[] = '<span class="fas fa-ban"></span><span class="fas fa-lock"></span> ' . Text::_('JLIB_RULES_NOT_ALLOWED_LOCKED');
                            }
                        }
                    }
                    elseif (!empty($component)) {
                        $html[] = '<span class="fas fa-check"></span><span class="fas fa-lock"></span> ' . Text::_('JLIB_RULES_ALLOWED_ADMIN');
                    }
                    else {
                        ...
                        elseif ($inherited === false) {
                            ...
                            $html[] = '<span class="fas fa-ban"></span><span class="fas fa-lock"></span> ' . Text::_('JLIB_RULES_NOT_ALLOWED_ADMIN_CONFLICT');
                        }
                        else {
                            $html[] = '<span class="fas fa-check"></span><span class="fas fa-lock"></span> ' . Text::_('JLIB_RULES_ALLOWED_ADMIN') ;
                        }
    ...
    protected function getSiteHTML($component, $section, $asset = '')
    {
        $html   = array();
        $html[] = '<div id="permissions-sliders">';
        $html[] = '<ul id="rules" class="list-group">';
        ...
        foreach($this->groups AS $i => $item)
        {
            ...
            $html[] = '<li class="list-group-item list-group-flush">';
            $html[] = '    <button type="button" class="btn btn-sm btn-mini" onclick="jQuery(\'#group-rules-' . $gid . '\').toggle();"><i class="fas fa-wrench"></i></button>';
            $html[] = '    <label for="">';
            ...
            foreach ($this->actions as $action)
            {
                ...
                $field_id    = $this->id . '_' . $action->name . '_' . $gid;
                $field_name  = $this->name . '[' . $action->name . '][' . $gid . ']';
                $field_desc  = htmlspecialchars(Text::_($action->title) . '::' . Text::_($action->description), ENT_COMPAT, 'UTF-8');
                $field_title = Text::sprintf('JLIB_RULES_SELECT_ALLOW_DENY_GROUP', Text::_($action->title), trim($item->text));
                ...
                if ($item->parent_id > 0) {
                    ...
                    if (Access::checkGroup($gid, 'core.admin') !== true) {
                        if ($inherited === null) {
                            $html[] = '<span class="badge bg-warning text-white"><i class="text-white fas fa-times"></i> ' . Text::_('JLIB_RULES_NOT_ALLOWED') . '</span>';
                        }
                        elseif ($inherited === true) {
                            $html[] = '<span class="badge bg-success"><i class="text-white fas fa-check"></i> ' . Text::_('JLIB_RULES_ALLOWED') . '</span>';
                        }
                        elseif ($inherited === false) {
                            if ($rule === false) {
                                $html[] = '<span class="badge bg-warning text-white"><i class="text-white fas faa-times"></i> ' . Text::_('JLIB_RULES_NOT_ALLOWED') . '</span>';
                            }
                            else {
                                $html[] = '<span class="badge bg-danger"><i class="text-white fas fa-lock"></i> ' . Text::_('JLIB_RULES_NOT_ALLOWED_LOCKED') . '</span>';
                            }
                        }
                    }
                    elseif (!empty($component)) {
                        $html[] = '<span class="badge bg-success"><i class="text-white fas fa-check"></i> ' . Text::_('JLIB_RULES_ALLOWED_ADMIN') . '</span>';
                    }
                    else {
                        // Special handling for  groups that have global admin because they can't be denied.
                        // The admin rights can be changed.
                        ...
                        elseif ($inherited === false) {
                            // Other actions cannot be changed.
                            $html[] = '<span class="badge bg-danger"><i class="text-white  fas fa-times"></i> <i class="fas fa-lock"></i> ' . Text::_('JLIB_RULES_NOT_ALLOWED_ADMIN_CONFLICT') . '</span>';
                        }
                        else {
                            $html[] = '<span class="badge bg-success"><i class="text-white fas fa-check"></i> <i class=" fas fa-lock"></i> ' . Text::_('JLIB_RULES_ALLOWED_ADMIN') . '</span>';
                        }
                    }
    ...
    protected function getActions($component, $section)
    {
        ...
        $actions = JPAccessHelper::getActions($component, $section);
    ...
    protected function getAssetRules($component, $section, $asset = '')
    {
        ...
        $this->rules    = Access::getAssetRules($asset_id);
