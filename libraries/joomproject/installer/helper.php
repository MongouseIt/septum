...
/**
 * Joomproject Installer Helper Class
 * Supports the install procedure of Joomproject related extensions
 *
 */
abstract class JPInstallerHelper
{
    /**
    ...
    {
        $asset_bak = Table::getInstance('Asset');
        $asset_new = Table::getInstance('Asset');
    ...
    /**
     * Method to set the admin component menu item as child of com_joomproject
     *
    ...
    {
        static $jp_menu_item = null;

        $db    = Factory::getDbo();
        $query = $db->getQuery(true);

        if ($element == 'com_joomproject') {
            return true;
        }

        // Find the joomproject admin component menu item
        if (is_null($jp_menu_item)) {
            $query->select('id')
                  ->from('#__menu')
                  ->where('menutype = ' . $db->quote('main'))
                  ->where('title = ' . $db->quote('com_joomproject'))
                  ->where('client_id = 1');

            $db->setQuery($query);
            $jp_menu_item = (int) $db->loadResult();
        }
        ...
        if (!$menu_item) {
            ...
            $menu->setLocation($jp_menu_item, 'last-child');
    ...
    /**
     * Method to add a menu item in the Joomproject site navigation menu
     *
    ...
    public static function addMenuItem($data)
    {
        ...
        if (!isset($data['menutype']))     $data['menutype']     = 'joomproject';
    ...
    /**
     * Method to register an extension to be uninstalled with com_joomproject
     *
    ...
    public static function registerCustomUninstall($element, $type = 'component')
    {
        ...
        // Get joomproject custom data
        $query->select('custom_data')
              ->from('#__extensions')
              ->where('element = ' . $db->quote('com_joomproject'))
              ->where('type = ' . $db->quote('component'));
