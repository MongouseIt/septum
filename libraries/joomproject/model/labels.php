 ...
 *
 */
class JPModelLabels extends ListModel
{
    /**
     * Constructor
     *
    ...
    public function getItems($project_id = 0)
    {
        ...
        if ((int) $project_id <= 0) {
            $this->setError(Text::_('COM_JOOMPROJECT_WARNING_LABELS_NO_PROJECT'));
            return array();
        }
    ...
    public function getConnections($item_type = NULL, $item_id = 0)
    {
        ...
        if (!$item_type || !$item_id) {
            $this->setError(Text::_('COM_JOOMPROJECT_WARNING_LABELS_NO_ITEM_REFERENCE'));
            return array();
        }
        ...
        $query->select('a.id, a.label_id, l.title, l.style')
              ->from('#__jp_ref_labels AS a')
              ->join('INNER', '#__jp_labels AS l ON l.id = a.label_id')
    ...
    public function save($data = array(), $project_id = 0)
    {
        ...
        $label    = $this->getInstance('Label', 'JPmodel', array('ignore_request' => true));
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $form = \Joomla\CMS\Factory::getApplication()->input->post->get('jform',[],'array');

        // Project id
        $project = (int) $this->getUserStateFromRequest('com_joomproject.project.active.id', 'filter_project', '');
