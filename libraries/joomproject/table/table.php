...
class JPTable extends Table
{
        ...
        if ($this->_trackAssets) {
            ...
            if ($this->_delete_children) {
                ...
                foreach ($all_assets AS $item)
                {
                    ...
                    $methods = JPTableHelper::getMethods($context);

                    if (!in_array('deletechild', $methods) && !in_array('deletebycontext', $methods)) {
                        continue;
                    }

                    $table = JPTableHelper::getInstance($context);
    ...
    {
        $allowed = JPAccessHelper::getAccessTree($new);
    ...
    protected function getForeignAssets($context)
    {
        ...
        JPTableHelper::discover();

        $assets   = array();
        $contexts = JPTableHelper::getContexts();

        foreach ($contexts AS $hash)
        {
            $methods = JPTableHelper::getMethods($hash);

            if (!in_array('getassetchildren', $methods)) continue;

            $table = JPTableHelper::getInstance($hash);
