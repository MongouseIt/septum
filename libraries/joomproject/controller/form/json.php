 ...
 *
 */
class JPControllerFormJson extends FormController
{
    /**
    ...
    public function batch($model)
    {
        ...
        if ($model->batch($vars, $cid, $contexts)) {
            $data['messages'][] = Text::_('JLIB_APPLICATION_SUCCESS_BATCH');

            $this->sendResponse($data);
        }
        else {
            $data['success']    = "false";
            $data['messages'][] = Text::sprintf('JLIB_APPLICATION_ERROR_BATCH_FAILED', $model->getError());
    ...
    public function cancel($key = null)
    {
        ...
        $recordId = \Joomla\CMS\Factory::getApplication()->input->getInt($key);
        ...
        if ($recordId) {
            ...
            if (!$this->checkEditId($context, $recordId)) {
                ...
                $data['success']    = "false";
                $data['messages'][] = Text::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $recordId);
    ...
    public function save($key = null, $urlVar = null)
    {
        ...
        // old one
        //$recordId = \Joomla\CMS\Factory::getApplication()->input->getInt($urlVar);
        // new one, as temporary solution , needs more investigations
        $recordId =  $data[$key];
        ...
        if (!$this->checkEditId($context, $recordId)) {
            ...
            $rdata['success']    = "false";
            $rdata['messages'][] = Text::sprintf('JLIB_APPLICATION_ERROR_UNHELD_ID', $recordId);
        ...
        if ($task == 'save2copy')
        {
            ...
            if ($checkin && $model->checkin($data[$key]) === false)
            {
                ...
                $rdata['success']    = "false";
                $rdata['messages'][] = Text::sprintf('JLIB_APPLICATION_ERROR_CHECKIN_FAILED', $model->getError());
        ...
        if (!$model->save($validData)) {
            ...
            $rdata['messages'][] = Text::sprintf('JLIB_APPLICATION_ERROR_SAVE_FAILED', $model->getError());

            $this->sendResponse($rdata);
        }

        // Save succeeded, so check-in the record.
        if ($checkin && isset($validData[$key]) && $model->checkin($validData[$key]) === false) {
            // Save the data in the session.
            $app->setUserState($context . '.data', $validData);
    ...
    public function reload($key = null, $urlVar = null)
    {
        ...
        $input   = (array) \Joomla\CMS\Factory::getApplication()->input->post->get('jform',[],'array');
