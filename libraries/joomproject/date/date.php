...
abstract class JPDate
{
    public static function relative($date = null, $tz = true)
    {
        ...
        if ($days >= 1) {
            if ($days == '1') {
                $format = Text::_('COM_JOOMPROJECT_DAY_' . ($is_past ? 'YESTERDAY' : 'TOMORROW'));
            }
            else {
                if ($days <= 7) {
                    ...
                    $format = Text::_('COM_JOOMPROJECT_DAY_' . ($is_past ? 'LAST_' : 'THIS_') . $day_names[$date_n]);
                }elseif($days >= 90){
                    ...
                    $format = Text::sprintf('COM_JOOMPROJECT_MONTHS' . ($is_past ? '_PAST' : ''), $sub_struct_month);
                }
                else {
                    $format = Text::sprintf('COM_JOOMPROJECT_DAYS' . ($is_past ? '_PAST' : ''), $days);
                }
            }
        }
        elseif ($hours >= 1) {
            $format = Text::sprintf('COM_JOOMPROJECT_HOUR' . ($hours > 1 ? 'S' : '') . ($is_past ? '_PAST' : ''), $hours);
        }
        elseif ($minutes >= 1) {
            $format = Text::sprintf('COM_JOOMPROJECT_MINUTE' . ($minutes > 1 ? 'S' : '') . ($is_past ? '_PAST' : ''), $minutes);
        }
        else {
            $format = Text::_('COM_JOOMPROJECT_MOMENT' . ($is_past ? '_PAST' : ''));
        }
    ...
    public static function shiftTimeline($timespan, $constraint = array(), $prev_constraint = array(), $options = array())
    {
        ...
        if ($item_has_start && strtotime($item_start_date) != $item_start_time) {
            $date = new Date(($item_start_time == 0 ? $nd : $item_start_time));
            $result[0] = $date->toSql();
        }
        else {
        ...
        if ($item_has_end && strtotime($item_end_date) != $item_end_time) {
            $date = new Date(($item_end_time == 0 ? $nd : $item_end_time));
            $result[1] = $date->toSql();
        }
        else {
